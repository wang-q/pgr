...
`ms2dna` is a subcommand of the `pgr` toolkit, used to convert haplotypes
...
```bash
# Read from file
pgr ms2dna input.ms -o output.fasta

# Read from standard input (pipe)
ms 3 1 -t 10 -r 0 1000 | pgr ms2dna > output.fasta
```
...
- **Note**: Due to differences in the underlying random number algorithms, the
  sequences generated by the Rust version of `pgr ms2dna` will differ from those
