...
- 文件中的坐标为 **1-based 起始、half-open 结束**（即结束位置不包含在内）。
- `pgr` 内部统一使用 **0-based half-open** 坐标。
- AXT 规范中 **target strand 恒为 `+`**，只有 query strand 可以为 `+` 或 `-`。
- 若 query 位于 `-` 链，文件中的坐标是反向互补后的相对坐标；转换到其它格式时会按各格式的约定处理。
...
- **坐标处理**:
  ...
  - 输出格式为 `>{t-name}.{chr}({strand}):{start}-{end}` 和 `>{q-name}.{chr}({strand}):{start}-{end}`，例如 `>target.chr1(+):11-14`。
- **参数**:
  - `chr.sizes`: 必需。Query 基因组的大小文件。
  - `--t-name`/`--q-name`: 自定义输出 FASTA 头中的物种名称前缀（默认分别为 `target` 和 `query`）。
...
- **高级选项**:
  - `--t-split`: 按 Target 序列名称将输出拆分到不同文件（输出为目录）。
    - 输入需按 target 名称排序或分组；若同一 target 非连续出现，其文件会被覆盖。
...
- **无效记录**: 坐标或序列无法构造有效 PSL 的记录会被跳过，并通过 `log::warn` 输出警告。
- **必需参数**:
  - `-t`/`--t-sizes`: Target 基因组大小文件。
  - `-q`/`--q-sizes`: Query 基因组大小文件。
