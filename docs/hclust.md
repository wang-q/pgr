# hclust 规划与与 UPGMA/NJ 的关系

本文记录 `hclust`（层次聚类）在 pgr 中的定位、与 UPGMA/NJ 的关系、常用方法（尤其是 `ward.D2`）的算法要点，以及推荐的工作流与实现规划。用于指导未来实现与文档对齐。

## 背景与定位
- 目标：提供通用的层次聚类（dendrogram）生成能力，支持 `single/complete/average/ward.D2` 等方法，输出 Newick 形式，便于下游 `nwk cut`。
- 范围：统计意义的 dendrogram（合并高度表达链接准则的代价），不强制“演化/分子钟”语义。
- 与 pgr 现有能力协同：
...
- 与 UPGMA 的关系：
  ...
  - 结论：两者链接更新一致，但语义不同；UPGMA 更偏系统发育场景，`hclust` 更偏统计聚类。
  - 参考实现：CLI [upgma](file:///c:/Users/wangq/Scripts/pgr/src/cmd_pgr/mat/upgma.rs)，库 [build::upgma](file:///c:/Users/wangq/Scripts/pgr/src/libs/phylo/build.rs#L1-L43)
- 与 NJ 的关系：
  ...
  - 参考实现：CLI [nj](file:///c:/Users/wangq/Scripts/pgr/src/cmd_pgr/mat/nj.rs)
...
- 生成树：
  ...
  - 一般加性距离场景：`mat nj`
  - 通用层次聚类分析或需要 `ward.D2`：`mat hclust --method ward.D2`
- 切分与评估：
...
## 实现规划（草案）
- CLI：`mat hclust`，`--method {single,complete,average,ward.D2}`，输入矩阵（pair TSV/Phylip），输出 Newick。
...
### 命令概览
- 名称：`pgr mat hclust`（可见别名 `hc`、`linkage`）
- 作用：从距离矩阵生成层次聚类树（dendrogram），输出为 Newick，便于后续 `nwk cut`。
- 归属：`mat` 模块，与 `upgma/nj` 并列。
...
pgr mat hclust matrix.phy --method ward.D2 > tree.nwk

# Average/complete/single（PHYLIP 输入）
pgr mat hclust matrix.phy --method average > tree.nwk
...
- 语义差异：
  - hclust 的合并高度是链接/准则的代价；不保证 ultrametric（除非数据满足相应条件）
