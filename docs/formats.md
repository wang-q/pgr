...
`pgr` 在 `src/libs/pairmat.rs` 中定义了三种核心的矩阵结构，用于处理聚类和距离分析。
...
*   **底层存储**：`Vec<f32>` (Flattened Array)。
*   **特点**：
    *   **紧凑存储 (Condensed Storage)**：仅存储上三角（不含对角线）元素，内存占用为 $N(N-1)/2$，即全矩阵的 $50\%$。
    *   **存储顺序**：行优先压缩，即 `(0,1), (0,2)...(0,n-1), (1,2)...`。
    *   **索引映射**：对于 $N$ 个元素，$(i, j)$ 且 $i < j$ 映射为 $k = N \cdot i - \frac{i(i+1)}{2} + (j - i - 1)$。
    *   **对称性与对角线**：强制对称，且假定对角线为 0（距离矩阵特性）。`get(i, i)` 始终返回 0.0。
    *   **纯数值**：不包含名称映射，专注于高性能计算。
