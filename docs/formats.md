...
| 特性 (Feature) | UCSC (`psl.c` / `struct psl`) | `pgr` (`libs/psl.rs` / `struct Psl`) | 备注 (Notes) |
| :--- | :--- | :--- | :--- |
| **字段对应** | `match`, `misMatch`, `repMatch`, `nCount` 等 unsigned int | `match_count`, `mismatch_count`, `rep_match`, `n_count` (u32) | 名称略有调整以符合 Rust 风格，类型一致 (u32)。 |
| **Insert 字段** | `qNumInsert`, `tNumInsert` (unsigned), `qBaseInsert`, `tBaseInsert` (signed int) | `q_num_insert`, `t_num_insert` (u32), `q_base_insert`, `t_base_insert` (i32) | 类型完全一致。注意 UCSC 中 BaseInsert 是 signed int。 |
| **坐标字段** | `qStart`, `qEnd`, `tStart`, `tEnd` (unsigned) | `q_start`, `q_end`, `t_start`, `t_end` (i32) | `pgr` 使用 `i32` 以适应可能的负值计算或通用性，但在 PSL 规范中通常是非负的。UCSC 使用 unsigned。 |
| **数组字段** | `blockSizes`, `qStarts`, `tStarts` (unsigned *) | `block_sizes`, `q_starts`, `t_starts` (Vec<u32>) | 动态数组实现一致。 |
| **Strand** | `char strand[4]` (通常是 2 字符 + null) | `String` | 语义一致。 |
| **输出格式** | 制表符分隔，数组字段以逗号分隔，末尾可能有逗号 | `write_to` 方法生成制表符分隔，数组字段以逗号分隔 | 输出格式保持严格一致。 |
...
1.  **整数类型**: UCSC C 代码主要使用 `unsigned` (32-bit)，`pgr` 显式使用 `u32`。UCSC 的 `BaseInsert` 字段是 `int` (signed)，`pgr` 使用 `i32` 匹配。
2.  **坐标类型**: `pgr` 的坐标字段定义为 `i32`，而 UCSC 是 `unsigned`。这在绝大多数基因组数据下没有影响，但 `i32` 理论上范围减半（20亿 vs 40亿）。考虑到大多数染色体长度小于 20亿，通常不是问题。但对于超大基因组可能需要注意。
3.  **读写支持 (Read/Write Support)**: UCSC `psl` 库支持完整的读写操作 (`pslLoad`, `pslWrite`)。目前 `pgr` 的 `libs/psl.rs` 仅实现了 `Psl` 结构体定义和写入 (`write_to`) 功能，主要用于 `lavToPsl` 等转换工具的输出，尚未实现 PSL 文件的解析读取功能。
