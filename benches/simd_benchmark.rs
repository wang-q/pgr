...
pub fn bench_rand(c: &mut Criterion) {
    ...
    c.bench_function("nalgebra_from_slice", |b| {
        let v = rand_vec(black_box(10005)); // Generate a random vector
        b.iter(|| {
            let _vec = nalgebra::DVector::from_column_slice(black_box(&v)); // Benchmark from_column_slice
        })
    });
}
...
pub fn bench_norm(c: &mut Criterion) {
    ...
    approx::assert_relative_eq!(norm_map(&v1), linalg::norm_l2(&v1), epsilon = 0.01);
    ...
    c.bench_function("norm_simd_pgr", |b| {
        b.iter(|| linalg::norm_l2(black_box(&v1)))
    });
...
// Historical reference (Ryzen 7 8745HS, hnsm/benches/simd.rs):
// msvc
