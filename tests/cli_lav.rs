...
fn test_lav_to_psl_trim_ends_bug() -> anyhow::Result<()> {
    ...
    if !input_path.exists() {
        eprintln!("Skipping test trimEndsBug because input file not found at {:?}", input_path);
        return Ok(());
    }
...
fn run_ucsc_test(name: &str) -> anyhow::Result<()> {
    ...
    if !input_path.exists() {
        eprintln!("Skipping test {} because input file not found at {:?}", name, input_path);
        return Ok(());
    }
    ...
    assert_eq!(output_lines.len(), expected_lines.len(), "Line count mismatch for {}", name);
