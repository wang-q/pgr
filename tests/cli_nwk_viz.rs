...
fn command_comment() -> anyhow::Result<()> {
    let bin = assert_cmd::Command::cargo_bin("pgr").unwrap();
    let mut cmd_color = std::process::Command::new(bin.get_program());
    ...
    let mut cmd_dot = std::process::Command::new(bin.get_program());
    let output = cmd_dot
        .arg("nwk")
        .arg("comment")
        .arg("stdin")
