...
use crate::libs::chain::{Chain, Block};
...
impl Fill {
    pub fn write<W: Write>(&self, writer: &mut W, indent: usize) -> io::Result<()> {
        ...
        for gap in &self.gaps {
            gap.borrow().write(writer, indent + 1, &self.o_chrom, self.o_strand)?;
        }
        Ok(())
    }
}
...
impl Gap {
    pub fn write<W: Write>(&self, writer: &mut W, indent: usize, o_chrom: &str, o_strand: char) -> io::Result<()> {
        let indent_str = " ".repeat(indent);
        writeln!(
            writer,
            "{}gap {} {} {} {} {} {}",
...
impl ChainNet {
    ...
    pub fn add_chain(&mut self, chain: Chain, min_space: u64, min_fill: u64) {
        ...
        if let Some(chrom) = self.chroms.get(&chain_rc.header.t_name) {
            ...
            add_chain_core(&mut chrom, chain_rc.clone(), blocks, false, min_space, min_fill);
        }
    }
    ...
    pub fn add_chain_as_q(&mut self, chain: Chain, min_space: u64, min_fill: u64) {
        ...
        if let Some(chrom) = self.chroms.get(&chain_rc.header.q_name) {
            ...
            add_chain_core(&mut chrom, chain_rc.clone(), blocks, true, min_space, min_fill);
        }
    }
}
...
fn add_chain_core(chrom: &mut Chrom, chain: Rc<Chain>, blocks: Vec<Block>, is_q: bool, min_space: u64, min_fill: u64) {
    ...
    for space in spaces {
        ...
        for i in start_block_idx..blocks.len() {
            let b = &blocks[i];
            let (b_start, b_end) = if is_q { (b.q_start, b.q_end) } else { (b.t_start, b.t_end) };
            ...
            if curr_s < s { s = curr_s; }
            if curr_e > e { e = curr_e; }
        }
        ...
        fill_space(chrom, space, chain.clone(), &blocks, first_idx.unwrap(), last_idx.unwrap(), s, e, min_space, is_q);
    }
}
...
fn fill_space(
    ...
    let o_chrom = if is_q { &chain.header.t_name } else { &chain.header.q_name };
    ...
    if fill_start > space.start && (fill_start - space.start) >= min_space {
        chrom.spaces.insert(space.start, Space {
            start: space.start,
            end: fill_start,
            gap: space.gap.clone(),
        });
    ...
    if fill_end < space.end && (space.end - fill_end) >= min_space {
        chrom.spaces.insert(fill_end, Space {
            start: fill_end,
            end: space.end,
            gap: space.gap.clone(),
        });
    ...
    for i in first_idx..last_idx {
        ...
        if gap_start > fill.borrow().start && gap_end < fill.borrow().end && (gap_end - gap_start) >= min_space {
             let (os, oe) = if !is_q {
                 (b1.q_end, b2.q_start)
             } else {
                 if chain.header.q_strand == '-' {
             ...
             chrom.spaces.insert(gap_start, Space {
                 start: gap_start,
                 end: gap_end,
                 gap: new_gap.clone(),
             });
            ...
            if !is_q {
                ...
                for d in &chain.data {
                    ...
                    if start < end {
                        ...
                        if qs < q_min { q_min = qs; }
                        if qe > q_max { q_max = qe; }
                    }
...
// Writing
pub fn write_net<W: Write>(chrom: &Chrom, writer: &mut W, is_q: bool, min_score: f64, min_fill: u64) -> io::Result<()> {
...
fn write_fill<W: Write>(fill: &Rc<RefCell<Fill>>, writer: &mut W, depth: usize, is_q: bool, min_score: f64, min_fill: u64) -> io::Result<()> {
    let f = fill.borrow();
    ...
    if sub_score >= min_score && sub_size >= min_fill {
        ...
        let mut line = format!("fill {} {} {} {} {} {} id {} score {:.0} ali {}",
            f.start, f.end - f.start,
            f.o_chrom, f.o_strand,
            f.o_start, f.o_end - f.o_start,
            f.chain_id,
            sub_score, sub_size
        );
...
fn write_gap<W: Write>(gap: &Rc<RefCell<Gap>>, parent: &Rc<RefCell<Fill>>, writer: &mut W, depth: usize, is_q: bool, min_score: f64, min_fill: u64) -> io::Result<()> {
    let g = gap.borrow();
    let p = parent.borrow();
