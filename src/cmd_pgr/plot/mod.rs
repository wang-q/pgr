...
pub fn make_subcommand() -> Command {
    Command::new("plot")
        ...
        .subcommand(heat::make_subcommand())
        .subcommand(dot::make_subcommand())
        .subcommand(nrps::make_subcommand())
        .subcommand(spectra::make_subcommand())
...
pub fn execute(args: &ArgMatches) -> anyhow::Result<()> {
    match args.subcommand() {
        ...
        Some(("heat", sub_matches)) => heat::execute(sub_matches),
        Some(("dot", sub_matches)) => dot::execute(sub_matches),
        Some(("nrps", sub_matches)) => nrps::execute(sub_matches),
        Some(("spectra", sub_matches)) => spectra::execute(sub_matches),
