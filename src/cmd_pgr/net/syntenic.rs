...
use pgr::libs::net::{read_nets, write_net, Chrom, Fill, Gap, range_intersection};
...
fn r_net_syn_gap(gap: &Rc<RefCell<Gap>>, map: &HashMap<String, DupeTree>, parent_fill: Option<&Rc<RefCell<Fill>>>) {
    let g = gap.borrow();
    for fill in &g.fills {
...
fn r_net_syn_fill(fill: &Rc<RefCell<Fill>>, map: &HashMap<String, DupeTree>, parent: Option<&Rc<RefCell<Fill>>>) {
