...
| bedtools  | BED/VCF/GFF 区间操作        | pgr 有 2bit/Chain/Net/PAF 等独特覆盖     |
...
所有用户输入（畸形数据、二进制文件、错误的命令行参数）必须返回友好错误，不能 panic。 这贯穿了
文件 I/O（`anyhow::Context` 附加错误上下文）、索引越界检查等所有层面。
...
- **JSON/YAML 序列化**：目前不需要，输出以 TSV/FASTA 为主
...
| [[benchmarks/bench-rg-runlist.md]] | `pgr rg runlist` vs `rgr runlist`：SpanIndex 二分后 overlap ~83×、superset ~588×（rgr 走旧 diff） |
