...
| 2026-06-28 | **V5 indel VCF 增量增强**：`pgr paf to-vcf` 从 substitution-only 扩展为 SNP + INS/DEL 三分支。主循环按 target 列是否 gap 分流：INS（target gap 连续段，1bp anchor + 插入碱基）、DEL（target 非 gap 段含 query gap，anchor + target 段 vs anchor）、SNP（单列替换）。DEL 分支加入 1bp anchor 修复完整删除样本 allele 为空的缺陷。POS 统一为 anchor 列的 1-based 坐标。无左对齐（建议 `bcftools norm` 后处理）。+2 集成测试（DEL/INS），共 44 paf tests。见 [[graph-design.md]] §4.2.3、§5.4 |
| 2026-06-28 | **`-m/--max-depth` 短选项补全**：`add_query_args` 中 `--max-depth` 加 `-m` 短选项（默认 2，0=unlimited）。库 `query_transitive_bfs` 早已实现深度控制，5 子命令（query/to-bed/to-maf/to-gfa/to-vcf）经 `add_query_args` 共享暴露，仅缺短选项。+2 集成测试（`-m 1` 短选项 + `-m 0` unlimited 三跳链），共 46 paf tests。graph-design.md §5.4 表格原记"BFS 默认无界"系过时文档，已订正为已完成。见 [[graph-design.md]] §5.4、§5.6 |
| 2026-06-28 | **`--syntenic-filter` 实现**：`add_query_args` 新增 `--syntenic-filter <chain_file>` 可选装饰器。加载 UCSC chain 文件，建 `(t_name, q_name) -> Vec<(q_start, q_end)>` 映射；对每个 query result 检查其 query 区间是否与对应 name 对的某个 chain query span 重叠，不重叠或无 chain 则丢弃。chain-level 粗粒度（不做 block 级双向覆盖、不做 Net 过滤），5 子命令经 `add_query_args` 共享。+2 集成测试（chain 覆盖保留/无 chain 过滤 + chain 存在但 query span 不重叠过滤），共 48 paf tests。见 [[graph-design.md]] §5.4、[[impg.md]] §6.8.4 |
| 2026-06-28 | **`--min-transitive-len` 文档订正**：确认 pgr 的 `--min-len`（默认 10）等同 impg 的 `--min-transitive-len`（默认 101）——两者语义均为"小于此长度的区间不进入下一轮 BFS"（index.rs L300 初始过滤 + L344 下一轮过滤）。仅命名与默认值差异，无需新实现。graph-design.md §5.4 表格订正为已完成。见 [[graph-design.md]] §5.4、[[impg.md]] §4.2 |
