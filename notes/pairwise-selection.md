...
```
pgr maf to-paf          # MAF → PAF 转换（数据源）✅ 已实现
pgr paf index           # PAF → 区间树索引（索引层，不过滤）✅ 已实现
pgr paf query <region>  # 区间投影 + 传递闭包 BFS（查询层，在此过滤）✅ 已实现
```
...
2026-06-27 | ✅ 三个命令全部实现：maf to-paf, paf index, paf query (89 tests)*
...
```bash
pgr paf index a.paf b.paf -o merged.paf.idx  # 多个 PAF 合并为一个索引
pgr paf query merged.paf.idx region --transitive  # 跨文件传递闭包
```

### 方案

impg 的三层架构（`Impg` → `MultiImpg` → `ImpgIndex` trait）的核心动机——
per-file 独立索引、per-target 懒加载、ForestMap 偏移量、缓存层——在 pgr V1
都不成立。详见 [[paf-implementation.md]] §14.6。

pgr V1 采用直接合并方案：

- `PafIndex::build_multi(readers: Vec<R>)` —— 收集所有文件的 PafRecords，
  统一 IndexMap 名字映射，合并 per-target interval map，构建统一 coitrees
- 不需要 ForestMap、TreeLocation、local_to_unified、RwLock、ImpgIndex trait

### 代码量

~30 行（build_multi 函数 + CLI 调整），3-4 个新测试。
2026-06-27 | 方向 A 持久化 (12 tests) + 方向 B 真实数据验证 (3 tests) + 覆盖率提升 (5 tests)；新增 impg 多文件架构分析
| 多文件索引方案 | ✅ 决策：直接合并（build_multi），不用 impg 三层架构 | impg 的 ForestMap/RwLock/ImpgIndex 在 pgr V1 都是过度设计 |
