...
2. **多 PAF 文件的统一索引**（✅ 已实现 `build_multi`）

   cohort 有多个 PAF 文件时，需统一装入同一个索引。pgr V1 采用直接合并方案——
   `PafIndex::build_multi` 收集所有文件的 PafRecords，统一 IndexMap 名字映射，
   合并 per-target interval map。详见 [[paf-implementation.md]] §14.6。
