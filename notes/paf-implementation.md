...
| P0     | 集成测试（`tests/cli_paf*.rs`）                        | `tests/`             | ✅ 已完成（58 tests，5 文件） |
...
- **bidirectional 双索引已实现**（V1 期间提前落地，原计划 V3）。impg 的 `from_multi_alignment_records`
  对每条 A→B record 额外生成一条 B→A entry，索引大小翻倍但免去反向查询的额外计算。pgr V1 已借鉴此设计：
  `+` 链 record 在 `trees[target_id]`（正向）和 `reverse_trees[query_id]`（mirror）各插入一条，mirror
  条目的 CIGAR 经 `reverse_cigar` 反转并交换 I/D。BFS 查询时同时遍历 `trees[tid]` 和
  `reverse_trees[tid]`，无需坐标变换。负链 record 不建 mirror（坐标变换非平凡）。持久化格式 v3 已同步
  支持 `reverse_intervals` + `FlatCigar::LazyReversed`。
...
单跳查询 +BFS 传递闭包，`PafRecord` 12 完整字段（不懒加载 CIGAR），bincode 持久化。**已落地**：
BGZF 懒加载 CIGAR（`CigarStore::Lazy` + `noodles_bgzf::IndexedReader`）、双向索引
（`reverse_trees` + `CigarStore::LazyReversed` + `reverse_cigar`）、过滤参数（`--min-degree`、
`--min-chain-length`）。新增依赖 `coitrees` + `serde`/`bincode`。交付物：`pgr maf to-paf` +
`pgr paf index` + `pgr paf query`。

**V2（多文件 + 懒加载 + rayon）**：多文件统一索引用 `local_to_unified` 翻译表（借鉴 MultiImpg），
rayon 并行构建，`trees` 包 `Arc<RwLock<>>` 支持并发读（V1 的 `Arc` 仅为借用方便，无并发共享），
`PafRecord` 瘦身到 8 字段。

**V3（大 cohort 优化，按需开启）**：`remove_cached_tree` 显式内存管理（大
