...
| **V5** ✅ | `pgr paf to-vcf`（POA MSA→VCF，SNP + indel）| impg `-o vcf` 的子集 | ~390 |
...
### 4.2.3 V5 `pgr paf to-vcf` 设计（SNP + indel VCF via POA MSA）

**核心思路**：复用 V3 的 POA MSA 产物，遍历 MSA 列调用三类变异（SNP/INS/DEL）输出 VCF。
...
**VCF 生成**（每个 region，三分支主循环）：
1. `build_msa_entries` → POA MSA（`Vec<String>`，含 gap，第一条是 target）
2. 遍历 MSA column，按 target 是否 gap 分三类：
   - **SNP**（target 非 gap，无 query gap）：REF = target base，ALT = distinct 非 REF ACGT base。POS 指向当前列。
   - **INS**（target gap 连续段）：anchor = gap 前一列的 target base。REF = anchor，ALT = anchor + 各样本插入碱基。POS 指向 anchor 列。
   - **DEL**（target 非 gap 连续段，≥1 query gap）：anchor = 段前一列的 target base。REF = anchor + target 段，ALT = anchor + 各样本非 gap 碱基（全删样本 ALT = anchor）。POS 指向 anchor 列。
3. POS = `target.start + t_aln_pos`（1-based；`t_aln_pos` 仅对 target 非 gap 列递增，INS 不递增）
4. GT：REF→`0`，ALT[i]→`i+1`，gap/非 ACGT→`.`
5. 仅当存在至少 1 个 ALT 时输出行

**Header**：`##fileformat=VCFv4.2` + `##FORMAT=<ID=GT,...>` + `#CHROM...`（sample 列为 entries 的
name 顺序，target 第一）

**与 `pgr fas to-vcf` 的关系**：逻辑同构（都遍历 MSA column 调变异），但输入源不同——
`fas to-vcf` 读 block FA（已 MSA），`paf to-vcf` 自己跑 POA 生成 MSA。未来可考虑提取共享的
"MSA→VCF" 工具函数，但当前两处实现都很短，重复可接受。

**已知限制**：
- **无左对齐**：INS/DEL 按 POA 产出的 MSA 列位置输出，不做 VCF 规范的左对齐。复杂 indel
  可能产生等价但非规范表示，可用 `bcftools norm -f ref.fa` 后处理规范化。
- **1bp 锚定边界**：若 indel 位于 MSA 首列（无前导 anchor 列）或前导列也是 gap，该 indel
  被跳过（VCF 规范要求 REF 非空，必须有锚点）。实际场景极少触发。
...
有两条互补路径——**单图工程优化**（seqwish 路线）与**分区分批构建**（impg partition+lace 路线）：
...
impg 的 `partition` 命令把 cohort 切成互不重叠的 loci（跨多条序列的同源区段集合），
每个 locus 独立构建 GFA，最后用 `lace` 拼回。算法是"批量传递闭包 BFS + masking 去重"，
**不是简单按窗口切分**。pgr 若引入此模式，设计要点（参考 [[impg.md]] §6.8.4）：

- **`partition_size: Option<usize>` 开关式触发** — 同一个 `pgr paf graph` 命令通过该字段
  切换"直接构建 vs 分区构建"，而非拆成两个子命令（impg 的设计实践）。
- **复用现有传递闭包** — partition 内部用的就是 `query_transitive_bfs`，pgr 已有该实现
  （`libs/paf/index.rs`），无需新算法。
- **lace 拼回** — 需新增 `pgr paf lace` 子命令，按 `NAME:START-END` 路径名约定拼装分区 GFA。

**判断标准**：先跑 4 万大肠杆菌基准测试（无优化版本）。若单图能跑通但慢 → 选路径 A 的 1-2 项；
若单图超 RAM → 选路径 B。不做"预防性优化"。
...
**`--smooth` 的参数参考**（来自 [[impg.md]] §6.8.1 的 `SmoothPipelineConfig` 默认值）：
`target_poa_lengths=[700,1100]`（两轮 POA，目标块长 700/1100 bp）、`max_node_length=100`
（节点最大长度，超长则切块）、`poa_padding_fraction=0.001`（POA 填充比例）、
`block_source=PathOverlap`（块来源：路径重叠）。pgr 若实现 `--smooth`，可直接复用 `libs/poa/`
的 SPOA 实现作为 POA 引擎，无需引入 spoasta/spoa_rs 外部依赖。

**`graph-report` 的维度参考**：impg `graph_report.rs` 输出 15+ 维度（节点数/边数/路径数/平均节点长度/
N50/最大 bubble 深度/路径覆盖率/重复节点比例等）。pgr 可先实现核心 5-6 维度（节点/边/路径/N50/
bubble 深度/覆盖率），按用户反馈扩展。
...
| **`--syntenic-filter`** | ✅ 已实现（chain-level query 侧覆盖检查，装饰器模式，5 子命令共享） | block-level 双向覆盖、Net 过滤（当前粗粒度够用） | — 已完成 |
| **`-m/--max-depth`** | ✅ 已实现（`add_query_args` 共享，默认 2，0=unlimited，5 子命令均暴露） | — | — 已完成 |
| **`--min-transitive-len`** | ✅ 等同 `--min-len`（默认 10，impg 默认 101） | — | — 已完成（命名差异） |
...
```
当前 (V1-V5) → 增量增强 (✅ indel VCF / ✅ -m/--max-depth / ✅ --syntenic-filter / ✅ --min-len≡--min-transitive-len)
            → V6 规模扩展 (4 万大肠杆菌基准 → 选 1-2 项 seqwish 优化)
            → V7 图质量 (graph-report → --smooth 评估)
            → V8 应用层 (genotyping，远期)
```

**近期焦点**：四项增量增强（indel VCF / -m/--max-depth / --syntenic-filter / --min-transitive-len 订正）全部落地；下一项为 4 万大肠杆菌基准测试（验证 V4a/V4b 可扩展性）。
