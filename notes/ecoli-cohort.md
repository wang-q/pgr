...
查询、GFA）已在 [[paf-route.md]] / [[graph-design.md]] 规划，本文档把它们串成一条端到端 pipeline。

参考文档：[[paf-route.md]]（PAF 隐式图核心目标与路线）、[[graph-design.md]]（V1-V5 实现）、
[[impg.md]]（impg 的 sparsify 与去冗余差异）、[[minigraph.md]]（粗框架哲学）。
...
PAF 索引 (隐式粗图)
    │
    │  [步骤 4: 查询] —— pgr paf query --transitive（V1-V3）
    ↓
同源区段列表 (BED/PAF/FASTA/MAF)
    │
    │  [步骤 5: GFA 物化] —— V4a 粗全局 + V4b 区域精细（可选）
    ↓
...
这与 [[paf-route.md]] §1 的"不重新做比对"原则一致——pgr 的核心价值在步骤 3-5
...
**为什么是 sparsify 而非全量**：见 [[paf-route.md]] §1.2——大 cohort + 无 MAF 先验时，
...
- `pgr paf index`（见 [[paf-implementation.md]]）
- 输入：步骤 2 产出的 pairwise MAF/PAF
- 输出：PAF 索引（隐式粗图）

### 2.3 步骤 4：查询（V1 已实现，V2-V3 待实现）

- `pgr paf query`（见 [[graph-design.md]] §3-§4）
- V1: `-o bed` 可选 + `-b` 批查（PAF 默认）✅ 已实现
- V2: `-o fasta`
- V3: `-o maf`（POA MSA）

### 2.4 步骤 5：GFA 物化（V4a ✅ / V4b-V5 待评估）

- V4a ✅: 粗全局 GFA（seqwish DSU 风格，见 [[graph-design.md]] §4.3.1）— 已实现 `pgr paf graph -f refs.fa --min-var-len 100`，输出 GFA v1.0（S/L/P），5 单元 + 7 集成测试覆盖正向/反向/大 indel 切分/小 indel 不切/阈值过滤
- V4b: 区域精细 GFA（impg 风格，见 [[graph-design.md]] §4.3.2）
- V5: 区域 GFA → MAF/VCF

**V4a 算法骨架**：seqwish 风格段级 DSU（CIGAR 切分 → 段对 → DSU 传递闭包 → 节点序列 → 路径
...
（单线程足够），路径方向恒 `+`（反向已翻转坐标到正链），rGFA SN/SO/SR tag 暂缺（待兼容性需要再加）。
...
落盘 mmap，内存吃紧时切到 disk-backed 模式，牺牲速度换可跑性。pgr V4a 处理 4 万大肠杆菌全图物化
（Gbp 级）时可考虑引入此兜底机制。详见 [[seqwish.md]] §2.3、§6.2。

---
## 3. 与 pgr 核心目标的关系

回到 [[paf-route.md]] §0 的核心目标——"复用 pairwise 资产，构建 PAF 隐式图"——4 万 E. coli
是这条路线的**应用场景**，不是目标本身。步骤 1（去冗余）是场景的前置条件，步骤 2（pair-selection）
是场景的规模约束，步骤 3-5 是 pgr 的核心能力。

**优先级**：pgr 的开发资源应集中在步骤 3-5（V1-V5 路线）。步骤 1 作为外部预处理，待 pgr 核心
能力稳定后再考虑是否封装（方案 B/C）。
...
- 2026-06-28：基于 [[seqwish.md]] 分析，补充 §2.1 不借鉴哈希稀疏化的理由，§2.4 V4a 算法骨架
  与磁盘后端兜底。
