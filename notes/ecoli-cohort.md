...
查询、GFA）已在 [[paf-pangenome.md]] / [[paf-pangenome.md]] 规划，本文档把它们串成一条端到端 pipeline。

参考文档：[[paf-pangenome.md]]（PAF 隐式图核心目标与路线）、[[paf-pangenome.md]]（V1-V5 实现）、
...
- `pgr paf query`（见 [[paf-pangenome.md]] §3-§4）
...
- V4a ✅: 粗全局 GFA（seqwish DSU 风格，见 [[paf-pangenome.md]] §4.3.1）— 已实现 `pgr paf graph -f refs.fa --min-var-len 100`，输出 GFA v1.0（S/L/P），5 单元 + 7 集成测试覆盖正向/反向/大 indel 切分/小 indel 不切/阈值过滤
- V4b: 区域精细 GFA（impg 风格，见 [[paf-pangenome.md]] §4.3.2）
- V5: 区域 GFA → MAF/VCF
