...
参考文档：[[paf-pangenome.md]]（PAF 隐式图核心目标、路线、V1-V5 实现）、[[impg.md]]（impg 的
...
### 2.3 步骤 4：查询（V1-V3 已实现）

- `pgr paf query` / `to-bed` / `to-maf`（见 [[paf-pangenome.md]] §3）
- V1 ✅: PAF 默认输出 + `-b` 批查 + `to-bed` 独立子命令
- V2 ✅: `to-maf` pairwise MAF（按 CIGAR 还原，需 `-f TSV`）
- V3 ✅: `to-maf --msa` POA 多序列 MSA（需 `--transitive`）

### 2.4 步骤 5：GFA 物化与 VCF（V4a/V4b/V5 已实现）

- V4a ✅: `pgr paf graph -f refs.fa --min-var-len 100` 粗全局 GFA（seqwish DSU 风格）
- V4b ✅: `pgr paf to-gfa` 区域精细 GFA（impg 风格，unchop 默认开，`--crush` 可选）
- V5 ✅: `pgr paf to-vcf` POA MSA → VCF（SNP + INS/DEL，1bp anchor）
