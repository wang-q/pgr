...
### 2.4 步骤 5：GFA 物化（V4-V5，待评估）

- V4a: 粗全局 GFA（minigraph 风格，见 [[graph-design.md]] §4.3.1）
- V4b: 区域精细 GFA（impg 风格，见 [[graph-design.md]] §4.3.2）
- V5: 区域 GFA → MAF/VCF

**V4a 算法骨架**：直接复用 seqwish 的 6 阶段流程（spanning tree → BFS discovery → DSU union-find
→ compact → links → GFA），详见 [[seqwish.md]] §6.2。pgr 输入 PAF 与 seqwish 一致，相对 minigraph
（需自跑 minimizer chaining）更天然适配。
