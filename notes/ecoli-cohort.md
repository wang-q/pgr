...
> **更新（2026-08-02）**：pgr 现已提供原生 `pgr align pgi`（两 `.pgi` 归并 →
> 链化 → banded 扩展 → PSL，与 FastGA 端到端 1.08× 持平、真实并集覆盖基本
> 一致，见 [[design/pgi-align.md]]），可作为本阶段 FastGA 的替代或内部对照；
> `pgr dist pgi` / `dist hv` 也可作 1a 之后的确定性/近似精筛层。
