...
- `best_crossover` 在 `fas_multiz` merge 的试点输出质量需真实数据验证。
  ✅ **2026-08-12 已闭环**：multi6 4 路酵母真实数据 + 130 kb 真实 block 受控
  冲突测试——拼接后参考 100% 一致、单侧物种 100% 保留；同时暴露窗口合并编排
  在真实多两两输入下覆盖丢失 87–100%，已按 multiz.c 块流合并重写修复
  （16/16 染色体覆盖恢复 100%，参考碱基与真实 multiz 100% 一致；详见
  `design/fas-multiz.md` §6）。
