...
**V4a rGFA tag** ✅：已在 `PafGraph::write_gfa` 的 S 行追加 `SN:Z`（源序列名）、`SO:i`（0-based 起始
偏移）、`SR:i:0`（rank 0 = primary）。origin 取 DSU 等价类中 `(seq_id, start)` 最小者（PAF target 先
注册，故 target 优先），novel 节点 origin 取其填充时的 `(name, start)`。SR 暂恒为 0（pgr 路径方向
恒 `+`，无链翻转）。与 minigraph rGFA / odgi 工具链兼容。
