...
V1-V6（graph-report）已全部完成。综合参考：impg（POA → GFA）、minigraph（chain → CIGAR → rGFA）、 seqwish（DSU
...
flumble 等重型设施。`--smooth` 视用户需求评估。完整 crush 不做。
...
**近期焦点**：§5.0 打磨项 + V6 剩余项（`--smooth` 视用户需求评估）。V7 规模扩展待 4 万大肠杆菌数据
可用后启动。V8 视用户反馈与真实数据验证后再启动。
