...
**PAF 是默认输出，BED 为 `-o bed` 可选**：impg `query` 默认 `-o bed`，pgr query 选择**PAF 为默认**
...
- `cli_paf_graph.rs`（9）— `paf graph`（全局图）
- `cli_paf_stat.rs`（6）— `paf stat`（25 维度统计）
- `cli_paf_to_bed.rs`（4）— `paf to-bed` + indel 坐标精度
- `cli_paf_to_fas.rs`（10）— `paf to-fas`（block FA 输出）
- `cli_paf_to_gfa.rs`（7）— `paf to-gfa`（局部图）+ path round-trip + lowercase round-trip
