...
`integration_test.rs` / `test/HLA/`（黄金回归），pgr `tests/cli_paf_query.rs` 的覆盖存在以下盲区：
...
测试位置：`tests/cli_paf_query.rs`（4 个 `command_paf_transitive_*` 函数，均用 `to-bed` +
`--transitive` 子命令验证）。
...
已落地 3 个测试（`command_paf_to_gfa_roundtrip_*`）：identical / SNP bubble / 2bp insertion bubble，
...
2 个测试（`command_paf_to_bed_*_coordinate_accuracy`）：insertion（`50=10I50=`，验证 insertion 边界
