...
V1-V6（stat）已全部完成。综合参考：impg（POA → GFA）、minigraph（chain → CIGAR → rGFA）、
...
```
索引层 ✅  |  查询层 ✅  |  图构建层 ✅（V4a 粗全局 + V4b 局部精细 + V5 VCF + V6 stat）|  应用层 ← 远期
```
...
| **V6** ✅  | `pgr paf stat`             | 粗图拓扑报告（25 维度 TSV）                | 复用 V4a `PafGraph::build`；节点长度/覆盖分布 + 连通分量 + tips/self-loop + 路径长度分布   |
...
§5.0 打磨项中 `--min-tree-coverage` 需 phylogeny 上下文。V6 `stat` 已完成。V7 规模扩展
