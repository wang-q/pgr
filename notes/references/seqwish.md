...
[main.rs](../../../seqwish-master/src/main.rs) 把整个流程串成 6 个阶段，
每阶段对应一个模块：

1. **序列索引** — [seqindex.rs](../../../seqwish-master/src/seqindex.rs)
...
4. **节点压缩** — [compact.rs](../../../seqwish-master/src/compact.rs)
   `compact_nodes`：在图序列上标记节点边界（分叉/汇合点），把无分叉的线性段压成单节点。
5. **边派生** — [links.rs](../../../seqwish-master/src/links.rs)
...
[pos.rs](../../../seqwish-master/src/pos.rs) 定义了贯穿全项目的位置类型：
...
[seqindex.rs](../../../seqwish-master/src/seqindex.rs) 把所有输入序列拼接
...
[intervaltree.rs](../../../seqwish-master/src/intervaltree.rs) 定义了
...
- [dset64.rs](../../../seqwish-master/src/dset64.rs) — `DisjointSets`，
...
- [dset64_asm.rs](../../../seqwish-master/src/dset64_asm.rs) —
...
- [dset64_unsafe.rs](../../../seqwish-master/src/dset64_unsafe.rs) —
...
**实际选用**：
[transclosure.rs#L12](../../../seqwish-master/src/transclosure.rs#L12)
...
## 4. 传递闭包：seqwish 的算法核心

[transclosure.rs](../../../seqwish-master/src/transclosure.rs)
...
#### 5.1.1 两套 AtomicBitVec 实现

seqwish 在两处独立实现了 `AtomicBitVec`，**接口不同、语义不同**，没有复用：

- [transclosure.rs#L72-L110](../../../seqwish-master/src/transclosure.rs#L72-L110)
...
- [compact.rs#L19-L77](../../../seqwish-master/src/compact.rs#L19-L77)
...
[compact.rs](../../../seqwish-master/src/compact.rs) 里有一处少见的 硬
panic：
...
### 5.2 derive_links：沿输入序列走边

[links.rs](../../../seqwish-master/src/links.rs) 的思路：
...
[gfa.rs](../../../seqwish-master/src/gfa.rs) 在写 P 行之前对每条
...
**关键认识**：`IITreeHandle` 用 `RwLock` 而 `AlnIITreeHandle` 用 `Mutex`，反映了 §3.3 的读写模式差异——节点/路径树构建后只读（多线程并发查询），比对树写入阶段需独占。`SeqIndexHandle` 用
...
#### 7.6.1 tempfile.rs：全局临时文件管理

[tempfile.rs](../../../seqwish-master/src/tempfile.rs) 用
...
#### 7.6.2 mmap.rs：手写内存映射

[mmap.rs](../../../seqwish-master/src/mmap.rs) 直接封装 `libc::mmap`，
...
#### 7.6.3 dna.rs：256 字节查表的碱基操作

[dna.rs](../../../seqwish-master/src/dna.rs) 用一个 256 字节的
...
#### 7.6.4 cigar.rs：极简 CIGAR 解析

[cigar.rs](../../../seqwish-master/src/cigar.rs) 定义
...
#### 7.6.5 paf.rs：PAF 行 + 多文件 spec 解析
...
[utils.rs](../../../seqwish-master/src/utils.rs) 两个函数：
...
#### 7.6.8 time.rs：单函数时间戳

[time.rs](../../../seqwish-master/src/time.rs) 只有
...
#### 7.6.9 version.rs：构建时注入版本号

[version.rs](../../../seqwish-master/src/version.rs) 用
...
- **`AtomicU64` + `fetch_or`** — `AtomicBitVec`（§5.1.1）的并行位标记。x86 编译成 `LOCK OR`，
  单指令原子地"置位并返回旧值"，用于 BFS 去重（transclosure）和并行边界标记 （compact）。比
  `Mutex<HashSet>` 快 10-100x。
- **`AtomicU128` + `compare_exchange`** — `DisjointSets`（§3.4）的无锁 union-find。 x86_64 编译成
...
#### 7.7.6 错误处理：io::Result + eprintln 诊断 + panic 兜底
...
- **SXS 格式**（[sxs.rs](../../../seqwish-master/src/sxs.rs)）： seqwish
...
- **`--sparse-factor` 哈希稀疏化**
  （[alignments.rs](../../../seqwish-master/src/alignments.rs)的
  `keep_sparse`）：seqwish 用哈希函数随机丢弃对齐，pgr 走 Mash KNN 稀疏化（见 [[ecoli-cohort.md]]），
  质量更高，不用哈希稀疏化。

## 9. 参考链接

- 源码：[seqwish-master/src/](../../../seqwish-master/src/)
