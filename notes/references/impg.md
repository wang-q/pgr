...
是**局部按需**物化——每次查询从一个区间出发。pgr graph 走 seqwish 路线（粗全局 GFA），to-gfa 走 impg 路线
（区域精细 GFA），同时利用两种粒度。
...
pgr query 已借鉴此设计 落地 `reverse_trees` 双向索引（仅 `+` 链建 mirror，负链不建；见
...
`M` 保持不变。这是 pgr mirror index 设计可借鉴的细节——pgr query 的 `reverse_trees`仅 `+` 链建 mirror
