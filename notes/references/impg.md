...
> 整理于 2026-06，源自对 impg-0.4.1 全部源文件的通读。目的：理解 impg 的隐式泛基因组图设计与命令分发架构，为 pgr 的泛基因组路线提供参考。
...
all-vs-all 比对 →seqwish 诱导图 → smoothxg 平滑（详见 [[smoothxg.md]]）→ 全图分析。这种路线的代价是：
...
- **UCSC Chain/Net 体系**：每条 Chain 是单条 A↔B 线性映射。`pgr psl lift` 把目标区间沿一条 Chain
...
**物化图内部的粒度差异**：seqwish（PGGB 路线）的物化是**全局一次性**的——一次性算出全部等价类再 写图
（spanning tree → BFS discovery → DSU union-find，详见 [[seqwish.md]] §4）；impg 的 `query -o gfa`
是**局部按需**物化——每次查询从一个区间出发。`pgr paf graph` 走 seqwish 路线（粗全局 GFA），`pgr paf to-gfa` 走 impg 路线
（区域精细 GFA），同时利用两种粒度。
...
- **coitrees** — cache-oblivious interval trees，impg 索引层的物理基础。按 target sequence 分多棵 树（
  [`TreeMap = FxHashMap<u32, Arc<BasicCOITree<QueryMetadata, u32>>>`](../../../impg-0.4.1/src/impg.rs#L226)），
  支持高效区间重叠查询。详见 §3.1。
...
- **bubble（气泡）** — 泛基因组图中的变异结构：从一个节点分叉出多条路径再汇合。crush 算法的
  处理对象 （§6.5）。
...
- **GFA engine** — 图构建引擎枚举
  （[`GfaEngine`](../../../impg-0.4.1/src/lib.rs#L38)）：`Pggb`/
...
`main.rs` 中的 [`detect_syng_prefix`](../../../impg-0.4.1/src/main.rs#L4604)
...
- **图构建层（泛基因组核心）** — `resolution.rs`（crush 算法实体，§6.5）+ `smooth.rs`（smoothxg
  风格块分解 + SPOA 平滑，smoothxg 算法详见 [[smoothxg.md]]）+ `graph_report.rs`（15+ 维度图质量报告，为 sweep 脚本提供评分依据）+
...
- **比对能力（aligner 路由）** — `lib_wfa2`（WFA 仿射比对，BiWFA 边界精修）+ `spoa_rs`/`poasta`
  ...
  aligner 之间做选择。
- **工具链集成（同团队生态）** — `sweepga`/`seqwish`/`allwave`/`gfasort`/`bluntg`/`povu` 都是
  Erik Garrison 团队维护的泛基因组工具，**作为库直接嵌入而非子进程调用**。这种"工具链库化"模式使
  impg 能深度集成各阶段，但也把外部工具的复杂性内化到二进制中。pgr 若引入类似生态，应评估"库化 vs
  子进程"的权衡。
...
impg 的子 crate 全部来自 Erik Garrison/Andrea Guarracino 团队的 pangenome 生态，
**除 gfaffix 外 均作为库直接嵌入**（与 §1.4 "工具链库化"一致）。
...
- **图构建生态（6 个，pangenome 团队）**
    ...
    - **seqwish**（pangenome/seqwish）— 从 PAF 诱导 GFA。 入口：
      ...
      调用位置：`graph` 的 Seqwish engine。**pgr 已对 seqwish 源码做独立分析**（[[seqwish.md]]），
      其算法骨架（spanning tree → BFS discovery → DSU union-find → compact → links → GFA）对 pgr
      graph 完全适用，pgr 输入 PAF 与 seqwish 一致。
    - **allwave**（pangenome/allwave）— crush 的
      [`Allwave` ResolutionMethod](../../../impg-0.4.1/src/resolution.rs#L8685)
        - syng-native BiWFA pair sparsification。调用位置：`crush`/`syng`。
    - **gfasort**（pangenome/gfasort）— GFA 排序（Ygs pipeline：path-guided
      SGD + grooming + 拓扑排序）。调用位置：`graph` 的 sort 阶段
      （[graph.rs#L423](../../../impg-0.4.1/src/commands/graph.rs#L423)）。
    - **bluntg**（pangenome/bluntg）— GFA bluntify（把 link/path
      overlap 转 0M）。调用位置：`syng2gfa --gfa-mode blunt`（
      [syng2gfa.rs#L3660](../../../impg-0.4.1/src/commands/syng2gfa.rs#L3660)）。

    - **povu**
      （povu-rs，pangenome/povu）— GFA 解析（`NativeGfa`）+ flubble 检测。调用位置：`graph report`（
      [graph_report.rs#L2135](../../../impg-0.4.1/src/graph_report.rs#L2135)）。

- **比对能力（3 个）**
    - **lib_wfa2**（ekg/lib_wfa2）— WFA 仿射比对（crush 边界精修 + syng-native BiWFA）。 调用位置：
      `crush`/`syng`。
    - **spoa_rs**（AndreaGuarracino/spoa-rs）— SPOA POA MSA 引擎。 调用位置：`gfa:poa` + crush
      阶段。
...
- **子进程调用（1 个）**
    - **gfaffix**（vendor/gfaffix）— **唯一子进程调用**（其余 14 个都是库嵌入）。 通过
      [`run_gfaffix`](../../../impg-0.4.1/src/graph.rs#L974) 用
      `current_exe().with_file_name("gfaffix")` 定位 sibling 二进制。作用：GFA 归一化（图构建后的
      normalize 阶段）。

**关键认识**：

- **库化 vs 子进程的不一致** — 14 个 git 依赖中 13 个库化嵌入，唯独 gfaffix 走子进程
  （[`run_gfaffix`](../../../impg-0.4.1/src/graph.rs#L974) 定位 sibling
...
- **pgr 可暂不引入整个生态** — pgr 第一步聚焦 Chain 传递闭包（paf-pangenome.md §3.1）， 只需
  coitrees 等价物（区间树）。sweepga/seqwish/gfasort 等图构建生态是后续阶段的需求。
...
[main.rs#L4707](../../../impg-0.4.1/src/main.rs#L4707) 处的
...
（[main.rs#L4892](../../../impg-0.4.1/src/main.rs#L4892)）
是整个项目最完整的输出格式说明，列出了 `auto/bed/bedpe/paf/gfa/vcf/maf/fasta/fasta+paf/fasta-aln/gbwt`
共 11 种输出与各自的约束。

### 2.3 命令分发模式 (`run` 函数)

[main.rs#L6168](../../../impg-0.4.1/src/main.rs#L6168) 处的
...
[main.rs#L164](../../../impg-0.4.1/src/main.rs#L164) 的
...
[main.rs#L1883](../../../impg-0.4.1/src/main.rs#L1883) 的
`silence_stdout_for_process`（unix 分支）通过 `RawFd` 重定向屏蔽子进程的 stdout，
用于 `gfaffix`、`odgi` 等外部工具的调用。这是 unix-only 代码，非 unix 平台有空实现（
[main.rs#L1926](../../../impg-0.4.1/src/main.rs#L1926)）。

### 2.5 测试入口

[main.rs#L6151](../../../impg-0.4.1/src/main.rs#L6151) 的
...
[impg.rs#L394](../../../impg-0.4.1/src/impg.rs#L394)
定义了单文件索引的核心结构：
...
[impg.rs#L165](../../../impg-0.4.1/src/impg.rs#L165) 的 `QueryMetadata`
...
[impg.rs#L243](../../../impg-0.4.1/src/impg.rs#L243) 的
...
[impg_index.rs](../../../impg-0.4.1/src/impg_index.rs) 定义了 `ImpgIndex`
...
另有独立的 `ImpgWrapper` enum（`Single(Impg)` / `Multi(MultiImpg)`）做 match dispatch， 本身不是
trait 方法。`MultiImpg` 内部维护 `TreeLocation { index_idx, local_target_id }` 把全局 `target_id`
翻译到子索引的本地 ID。`main.rs`中的命令代码只与 `&dyn ImpgIndex` 打交道，从而对单/多文件透明。

`MultiImpg` 还实现了 staleness 检测：当 `.impg` 索引比源比对文件旧时， 警告并要求 `--force-reindex`。

### 3.5 `AlignmentRecord`：三格式统一抽象

[`alignment_record.rs`](../../../impg-0.4.1/src/alignment_record.rs)
定义了 PAF/1ALN/TPA 三种比对格式的统一表示，是 §1.2 双后端架构的"格式抽象层"物理载体（§1.3 提及）。
上层索引与查询代码只与 `AlignmentRecord` 打交道，不感知具体比对格式：
...
pub struct AlignmentRecord {
    ...
    pub strand_and_data_offset: u64,  // MSB = strand, 低位 = data offset
...
- **PAF**：`strand_and_data_offset` 低位 = 文件偏移量（指向 `cg:Z:` 标签）；`data_bytes` = CIGAR
  字符串字节数。
- **1ALN**：`strand_and_data_offset` 低位 = alignment index（O(1) seek）；`data_bytes` = tracepoint
  数量。
- **TPA**：同 1ALN 的 tracepoint 编码。

**strand 编码** — `STRAND_BIT = 0x8000000000000000`（MSB），`strand()`/`set_strand()`
通过位运算读写，不占用额外空间。这与 `QueryMetadata`（§3.2）的 `strand_and_data_offset` bit 63 =
strand 设计一致——`AlignmentRecord` 是入库前的中间表示，`QueryMetadata` 是入库后的区间树节点表示，
两者共享 strand 编码方案。
...
impg 在 [impg.rs](../../../impg-0.4.1/src/impg.rs) 中提供
**两个版本的投影函数**，由 `query` 的 `approximate_mode` 参数切换：

- **`project_overlapping_interval`**
  （[L1098](../../../impg-0.4.1/src/impg.rs#L1098)）—完整模式：计算完整
  CIGAR + 序列 I/O，产出精确的 query 侧坐标与 `AdjustedInterval`。PAF 走 `cg:Z:` 解析，1ALN/TPA 走
  tracepoint 解码 + BiWFA。
- **`project_overlapping_interval_fast`**
  （[L1313](../../../impg-0.4.1/src/impg.rs#L1313)）—快速近似模式：
  **仅基于 tracepoint 统计做近似投影**，无 CIGAR 计算、无序列 I/O，identity 从 tracepoint
  统计估算。仅 1ALN/TPA 文件可用，PAF 无 tracepoint 退回完整模式。

这种"完整 vs 近似"双模式是 impg 处理大 cohort 时的性能开关——`--approximate` 在 fastga
`trace_spacing` 配合下可跳过 BiWFA 重建。pgr 只支持 PAF（无 tracepoint），query 不需要这个分叉。

`pgr psl lift` 本质上就是单条 Chain 的区间投影：通过 Chain 的坐标映射把目标区间 lift 到查询 序列。
区别在于 impg 在 all-vs-all 比对网络的并集上做区间树查找，而 pgr psl lift 在用户手动 指定的单条
Chain 上做线性投影。
...
| 维度     | impg 区间投影/传递闭包              | pgr psl lift                     |
|----------|-------------------------------------|------------------------------------|
...
复用 impg 的 BFS 传递闭包思路，实现 cohort 级"找全所有同源片段"——这是 pgr 当前 `psl lift`
（单链线性投影）的天然扩展。
...
选择 ([lib.rs#L38](../../../impg-0.4.1/src/lib.rs#L38)的 `GfaEngine`
枚举)：
...
**算法**
（[`partition_alignments`](../../../impg-0.4.1/src/commands/partition.rs#L158)，
核心循环 L295-L570）：
...
**避免 all-vs-all 比对的机制**（align 阶段）— impg 的 Graph/Query/Partition 命令在 align 阶段默认跑
self-PAF（all-vs-all），但提供 4 层参数控制"挑选哪些对比对"，避免 O(n²) 全对比：

    - **`--sparsify <策略>`**（
      [`GraphBuildConfig.sparsify`](../../../impg-0.4.1/src/commands/graph.rs#L98)，
      ...
      [main.rs#L3990](../../../impg-0.4.1/src/main.rs#L3990)
      验证逻辑："`--sparsify controls external-aligner pair selection`"。
      [graph.rs#L1153-L1243](../../../impg-0.4.1/src/commands/graph.rs#L1153)
    ...
    - **Syng 引擎**（`--gfa-engine syng`/`syng-local`）— 从 syng syncmer anchor counts 选对，
      [main.rs#L4009](../../../impg-0.4.1/src/main.rs#L4009)：
...
是基于覆盖度的贪心选择：跟踪 `missing_regions`，选"缺失最多"的序列开窗，当所有序列都被 覆盖时停止
（L906）。但 Partition 消费已有 PAF，不跑比对——它控内存的方式是"按 locus 切分 cohort"（详见
§6.3），而非"挑选对比对"。4. **图操作原语**（`graph.rs`）— `unchop_gfa`/`sort_gfa`（gfasort
Ygs 集成）/ `build_spoa_engine`/`feed_sequences_to_graph`（SPOA）/`terminal_n_clip_span`/
`prepare_poa_graph_and_sequences`。这些是各阶段共享的低层原语。5. **crush 阶段**（`resolution.rs`，
§6.5）— 由 `parse_crush_stage` 装配 `ResolutionConfig` 后调用 `resolve_gfa_bubbles`。
6. **smooth 阶段**（`smooth.rs`）— smoothxg 风格块分解 + SPOA 平滑，`SmoothConfig` 的
`block_source: SmoothBlockSource`（PathOverlap/Flubble/NeighborMergePoasta 三策略）决定块来源。7.
**后处理**（`gfa_self_loops.rs` + `commands/syng2gfa.rs`）— `NormalizeSelfLoops` 折叠 blunt GFA
...
[`resolution.rs`](../../../impg-0.4.1/src/resolution.rs)（17169 行，
全项目最大源文件）。
...
crush 的核心动机记录在
[crush-architecture-spec.md](../../../impg-0.4.1/docs/crush-architecture-spec.md)
...
代码中 [`ResolutionMethod`](../../../impg-0.4.1/src/resolution.rs#L274)
...
- **入口函数**
  [`resolve_gfa_bubbles`](../../../impg-0.4.1/src/resolution.rs#L997)
  接收 GFA 1.0 字符串（仅 `S`/`L`/`P`，link 必须 blunt `0M`），返回 `ResolvedGfa { gfa, stats }`。
- **`ResolutionConfig`** ([L35](../../../impg-0.4.1/src/resolution.rs#L35))
  是 crush 行为的完整参数化，按职责分六组（共 30+ 字段）：
    - **迭代控制** — `max_iterations`（默认 1，`until-done` = `usize::MAX`）、`method`
      （`ResolutionMethod`，§6.5.3）。
    - **Auto 三档路由阈值** — `auto_spoa_max_traversal_len`/`auto_poasta_max_traversal_len` （median
      阈值，设 0 禁用对应档）、`auto_allwave_max_total_sequence`/`auto_allwave_max_traversals`
      （legacy，仅 CLI 兼容保留）。
    - **bubble 规模守卫** — `max_bubble_span`（root path 最大跨度 bp）、 `max_traversal_len`/
      `min_traversal_len`（单次直接替换的遍历长度上下界；`min` 允许显式分层 pass：先 crush 大
      bubble 再跑小 bubble）、`max_median_traversal_len`/`max_total_sequence`/`max_traversals`（0
      禁用对应守卫）。
    - **polish（§6.5.2 阶段 7）** — `polish_iterations`/`polish_max_traversal_len`/
      `polish_max_median_traversal_len`/`polish_max_total_sequence`/`polish_max_traversals`控制小
      tangle POASTA 清理的规模预算，`polish_method`（`Poa`/`Poasta`/`Smooth` 三选一）。
    - **pairwise 引擎参数** — `abpoa_bin`（abPOA 可执行路径）、 `pair_k_nearest`/
      `pair_k_farthest`（多序列 pairwise 引擎的近邻/远邻采样数）、`pair_trees`（独立 tree-sampling
      pass 数，取并集）。
    - **motif-local（§6.5.3 后期补救）** — `motif_max_sparse_paths`/`motif_min_flank_paths`/
      `motif_min_order_jump`/`motif_max_window_bp`，专为 `MotifLocal` method 服务。
- **关键常量** ([L505-L569](../../../impg-0.4.1/src/resolution.rs#L505))：
...
- **path-preserving 校验（2 个）** —
  [`compare_gfa_paths.rs`](../../../impg-0.4.1/examples/compare_gfa_paths.rs)
  比较两个 GFA 路径拼写是否逐字节一致；
  [`validate_gfa_path_sources.rs`](../../../impg-0.4.1/examples/validate_gfa_path_sources.rs)
...
### 6.8 lib.rs：库入口与双后端适配器模式

`lib.rs` 仅 1994 行，是 impg 库入口（31 个 `pub mod` 声明），但更关键的是它汇聚了图构建层的
**公共类型与适配器**，是 §1.2 "双后端架构"的物理载体。前文 §1.3/§6.1/§6.4 已分别提及各类型，
本节集中梳理 lib.rs 的结构性角色。

#### 6.8.1 公共类型汇聚

lib.rs 顶层定义了贯穿图构建层的核心类型：

- **`GfaEngine` 枚举**（5 变体）— §6.1 已详述。值得补充的是 `SyngNative` 的 doc comment 完整描述了 6
  阶段 syng-native 管道（per-partition 距离矩阵 → KNN pair selection →strand groom → anchor-seeded
  BiWFA → indel left-align → emit PAF → seqwish），是 syng 后端免比对的算法骨架。`SyngLocal`
  则是"取查询子集 → 重建局部 syng syncmer 图 → 同套 blunt/crush/sort 变换"的快速通道。
- **`EngineOpts` 结构**（~15 字段）— 解析后的引擎配置。**关键字段 `partition_size: Option<usize>`**
  非空时触发"分区分批构建 GFA + lace 拼回 + 末尾一次 gfaffix"模式（§6.3），即同一个 `graph`
  命令通过此字段切换直接构建 vs 分区构建，而非拆成两个命令。
- **`SmoothPipelineConfig`** — post-crush smoothxg pass 的独立配置（与 pggb-engine 内嵌
  smoothing 区分），默认值（`target_poa_lengths=[700,1100]`、`max_node_length=100`、
  `poa_padding_fraction=0.001`）与 pggb 一致。
- **`DEFAULT_SYNG_GFA_SORT_PIPELINE = "Ygs"`** — syng-native GFA 默认走 gfasort 的 Ygs pipeline。

#### 6.8.2 双后端适配器：ImpgIndex trait 的两个实现

§1.2 的"双后端架构"（Alignment 后端 vs syng 后端）在 lib.rs 中通过两个 `ImpgIndex` trait
实现物理落地：

- **`SeqIndexWrapper`**（私有）— 最小适配器，只实现 `seq_index()` 返回 `&SequenceIndex`， 其余方法
  （`query`/`query_with_cache`/`query_transitive_*`/`get_or_load_tree` 等）全部返回 空值。专供
  `dispatch_gfa_engine_with_seq_index` 使用——当 query intervals 已由 syng 计算好、只需把 GFA
  引擎接到 SequenceIndex 上时，避免构造完整 ImpgIndex 的开销。
- **`SyngImpgWrapper`**（pub）— 完整适配器，把 `SyngIndex` 包装成 `ImpgIndex`。核心是
  `query_via_syng` 方法：把 `ImpgIndex::query` / `query_transitive_dfs` / `query_transitive_bfs`
  全部转发到 `SyngIndex::query_region`。**关键设计**：
    - **链过滤可选**（`with_chain_filter(min_anchors, min_fraction)`）— 启用时用 SweepGA plane-sweep
      链式化 syncmer 命中，按 anchor 数 + query 覆盖率过滤弱链/paralog 噪声，再交给 partition 的
      union-find；不启用时原始 per-syncmer 区间直通。
    - **`effective_min_chain_anchors_for_syncmer`**自适应阈值 — 根据 query 长度与 syncmer
      长度动态调整最小 anchor 数，避免短 query 误过滤。
    - **`get_or_load_tree` 返回 `None`** — syng 无 alignment interval tree，调用方需走 `query()`
      路径而非直接读树。
    - **`syng_index_ref()` 提供 `Option<&SyngIndex>`** — 让需要完整 `HomologousInterval` （含
      strand/genome name）的调用方绕过 `ImpgIndex` 抽象直接访问。

**关键认识**：`ImpgIndex` trait 抽象了"区间查询能力"，使 `partition`/`graph`/`genotype`
等 上层命令对"PAF 区间树 vs syng syncmer 索引"两种后端透明。**代价**是 trait 方法签名臃肿
（`query_transitive_bfs` 13 个参数），反映两种后端的最大公约数。pgr 若引入类似抽象，应优先
用更细粒度的 trait 组合（如 `IntervalIndex` + `TransitiveIndex` + `SequenceLookup`）而非 单一大
trait。

#### 6.8.3 GFA 引擎分发

`dispatch_gfa_engine_with_seq_index` 是 lib.rs 的核心公共函数：构造 `SeqIndexWrapper` 后转发到
`dispatch_gfa_engine(&wrapper, query_intervals, sequence_index, scoring_params, engine_opts)`。
后者根据 `engine_opts.engine` 分发到具体引擎实现（Pggb/Seqwish/Poa/SyngNative/SyngLocal），
返回 GFA 字符串。**这是命令层（`commands/graph.rs` 等）与引擎实现之间的唯一桥梁**，使命令层
不必感知具体引擎差异。

#### 6.8.4 对 pgr 的启示

1. **公共类型汇聚点** — impg 把跨模块共享的类型（`GfaEngine`/`EngineOpts`/ `SmoothPipelineConfig`）
   放在 lib.rs 而非独立 `config.rs`，是因为这些类型与引擎分发强耦合。pgr 的 `libs/paf/` 已有类似实践
   （`index.rs` 集中定义核心类型），可继续保持。
2. **双后端适配器模式的取舍** — impg 的 `ImpgIndex` 用一个胖 trait 抽象两种后端，签名臃肿。 pgr
   若未来要支持 PAF + Chain 双后端，应优先考虑 trait 组合而非单一大 trait。
3. **`partition_size: Option<usize>` 的开关式分区** — 同一个 `graph` 命令通过该字段切换 "直接构建
   vs 分区构建"，而非拆成两个命令。pgr 的 `pgr paf graph` 若未来需要分区模式，可借鉴这种"Option
   字段触发分支"而非"独立子命令"的设计。
4. **`SyngImpgWrapper::with_chain_filter` 的可选过滤层** — syng 后端的链过滤是可选的 （None
   时直通原始区间），这种"过滤作为可选装饰器"模式对 pgr 的 `--syntenic-filter`设计有参考价值：
   Chain/Net syntenic 过滤可作为查询层的一个可选装饰器，而非强制路径。

## 7. 应用层：基因分型与等位基因推断

应用层是能力栈的顶端消费者，依赖图构建层（§6）或隐式图查询（§4）提供 evidence。
impg 的 `genotype`/`infer` 命令实现基于 graph feature 的基因分型，核心设计在
[`docs/genotype-architecture.md`](../../../impg-0.4.1/docs/genotype-architecture.md)
与 [`docs/infer-design.md`](../../../impg-0.4.1/docs/infer-design.md)
（README 显式引用）。
...
**代码级实现** — [`genotyping.rs`](../../../impg-0.4.1/src/genotyping.rs)
用三个 enum 把"后端中立"从设计文档落到代码：

- **`FeatureSpace`**（6 变体）— 候选 haplotype 的特征表示空间： `SyngSyncmerNode`/`GfaSegment`/
  `VariationGraphNode`/`LocalHaplotypeSegment`/`HaplotypeSequence`/`MemHit`。前两个对应隐式图
  （syng）与物化图（GFA）两条后端路径。
- **`EvidenceBackend`**（6 变体）— sample evidence 的投影后端： `Pack`/`Projection`/`SyncmerWalk`/
  `VariationGraphAlignment`/`HaplotypeAlignment`/`MemProjection`。`Pack` 对应 syng 的 pack 索引，
  `Projection` 对应区间投影（§4.1）。
- **`ScoringMethod`**（5 变体）— 评分方法：`Cos`（cosine 相似度，当前实现）、 `CountLikelihood`/
  `AlignmentLikelihood`/`LearnedLocalEmission`/`GbwtImputation`（规划中）。
...
2. **传递闭包 ≠ 多序列比对** — 传递闭包只做"图遍历可达性查询"，不产出多重比对；间接同源 （A~B、
   B~C）会被报告，但三者的 MSA 需另走 `gfa`/`maf` 输出路径（机制详见 §4.3）。pgr 借鉴时
   需清醒认识"图遍历 vs 多序列比对"的本质区别。
...
- **Chain 是 star topology**（ref↔query_i），不是 all-vs-all——要做传递闭包必须跨多条 Chain， ref
  成为必经枢纽，ref 缺失区段会断开间接同源路径。
...
1. **MAF → PAF** — pgr 已有生成两序列 MAF 的能力。每个 MAF block 就是一条 pairwise alignment，
   可序列化为 PAF 行（target/query 名称 + 起止坐标 + 链向 + `=`/`X` CIGAR）。这是 pgr 已有 pairwise
   基础设施通往 PAF-based 隐式图的天然桥梁，无需引入 wfmash。
2. **PAF → 区间树** — 把全 cohort 的 PAF（含直接 MAF→PAF 转换 + 跨 sample 比对）按 target seq
   ...
   （§3.2）：query_id/target_start/target_end/query_start/query_end/strand/data_offset。CIGAR 的
   indel delta 懒加载，只存文件偏移量。
3. **`pgr paf query` 升级为传递闭包** — 在区间树之上实现 `--transitive` （对应 impg
...
**查询层过滤**（syntenic 净化、低质量剪枝），但不适合做图边集本身。

**前提设施**：

- `pgr maf to-paf` 已实现（见 §9.4），可作为 PAF 索引的前置转换。
...
> [paf-pangenome.md](../../../notes/paf-pangenome.md)，112 个集成测试通过）：

> - ✅ `pgr maf to-paf`（
  [src/cmd_pgr/maf/to_paf.rs](../../../src/cmd_pgr/maf/to_paf.rs)）
> - ✅ `pgr paf index`（
  [src/cmd_pgr/paf/index.rs](../../../src/cmd_pgr/paf/index.rs)）
> - ✅ `pgr paf query`（
  [src/cmd_pgr/paf/query.rs](../../../src/cmd_pgr/paf/query.rs)）
> - ✅ `--transitive` BFS 传递闭包
  （[src/libs/paf/index/](../../../src/libs/paf/index/)）
> - ✅ 查询层过滤参数 `--min-identity`/`--min-output-len`/`--merge-distance`/`--max-depth`
> - ✅ 已实现：`pgr paf query -o bed`（可选，PAF 默认）+ `-b regions.bed` 批查
...
3. **`pgr paf query <region>`** — 在区间树上查找所有重叠 PAF record，把目标坐标 lift 到查询序列，输出
   BED（最简输出）。查询参数暴露过滤：`--merge-distance`/`--min-identity`/`--min-output-len`/
   `--subset-sequence-list`（对应 impg `QueryOpts` 的 `-d`/`--min-result-identity`/`-l`/
...
5. **`pgr paf query <region> --transitive -o maf`** — 传递闭包结果 + 局部 MSA，复用
   `libs/fas_multiz/` 输出 MAF。这是"隐式图 + 按需物化"的最小闭环。
...
避免 all-vs-all 比对——用 Mash KNN 选对再跑 wfmash。pgr 的场景是**已有两序列 MAF**（可转 PAF），
天然避开了 all-vs-all 问题。pgr 的"挑选"发生在查询层（`pgr paf query` 的 `--merge-distance` 等参数），
...
* **2.36× 更多序列**，crush 仍未追平 PGGB 的压缩率。 实验也记录**假设被反驳**的情况
（`crush-exp-sweepga-k31`：降低 seqwish-k 不能 rescue 小 bubble；`crush-exp-min-run-5`：min-run=5
与 min-run=3 mask 掉完全相同节点）——这种"明确记录失败"的纪律 值得借鉴。
...
metrics.tsv 数据文件）。`validate-current-c4.md` 的结论 "better than prior SYNG-derived C4
outputs on the specific repeat-artifact failure mode, but still not solved and not yet..."反映 C4
难用例至今未完全解决。
...
**`local-compression-testbed-fast/` — 测试床 fast profile** — 含 `scoreboard.json`（每
fixture ×method 的 `fixture_class`/`tier`/`method_family`/`method_parameters`/`command_line`/
...
- **可借鉴**：impg 的 `crush-architecture-spec.md` 模式——当某个算法复杂到"three days of agents
  ...
  change should be proposed without referencing this document"。`pgr` 的 `notes/references/spoa.md`
  等移植笔记已有类似性质，可强化。
