...
3. **`pgr maf query`/`pgr paf query` 升级为传递闭包** — 在区间树之上实现 `--transitive` （对应 impg
...
> [paf-pangenome.md](file:///Volumes/ExtHome/Scripts/pgr/notes/paf-pangenome.md)，104 个测试通过）：
...
> 输出 GFA v1.0（S/L/P），seqwish DSU 风格（详见 [[paf-pangenome.md]] §4.3.1、[[seqwish.md]]
