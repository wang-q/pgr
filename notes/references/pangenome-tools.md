...
pgr graph 的直接借鉴来源（见 [[minigraph.md]]）。
...
两者互补而非竞争。pgr to-vcf+ 可考虑把 Pangene 作为上层接口——查询某基因时，先用 Pangene 定位子图，再用
pgr 查碱基级细节。当前路线不纳入，但记录为远期可能。

## 5. 对 pgr 路线的启示

1. **seqwish 对照已完成** — [[seqwish.md]] 详细拆解了 seqwish 的 6 阶段流程（序列索引 → 比对索引 →
   传递闭包 → 节点压缩 → 边派生 → GFA 输出），明确 pgr graph 可直接复用其算法骨架（spanning tree →
   BFS → DSU → compact → links → GFA），query 可借鉴 `PosT` 编码与 `SparseBitVec`，to-gfa 可借鉴
