...
### 5.5 FastCodeML
*   **定位**: PAML CodeML 的高性能 C++ 重写版，专为高性能计算集群 (HPC) 设计。
*   **核心文献**: [Valle et al. (2014) Bioinformatics](https://doi.org/10.1093/bioinformatics/btt760) - 详细阐述了通过算法优化（如子树复用）和并行化实现的加速效果。
*   **特点**:
    *   **并行计算**: 原生支持 OpenMP (单机多核) 和 MPI (多机集群)，可利用数百个 CPU 核心并行加速。
    *   **性能**: 相比原版 CodeML，单核速度提升约 5.8 倍，多核并行下可实现近线性的加速比（如 96 核加速 170 倍）。
    *   **算法优化**: 实现了**子树复用 (Subtree Reuse)** 技术，避免对相同序列模式的重复计算。
    *   **局限性**: 侧重于 Branch-Site 模型，安装配置较复杂（依赖 BLAS, LAPACK, MPI 等），且不如 Godon 易用。
...
> **状态：计划中（未实现）** — 以下路径与代码结构均为设计稿，`src/libs/kaks/` 尚未实现。
...
*   **目标**: 支持基因组级扫描。
*   **任务**:
    1.  **滑窗分析**: 结合 `pgr fa window` 和内置 Ka/Ks 计算，实现全基因组滑窗选择压力扫描。
