...
`fq`（10 子命令）命令族合计修复本次缺陷 3 类（Zero-Panic / clap 参数缺失 2
[split、sample] + `--parallel` 范围校验 1 [clean/filter/norm/clump 四处]），
均含回归测试；另有承袭自 `audit-fa-fq-2bit.md` 的数据安全 1、输入校验 1、行为
一致性/算法 2、文档一致性 5。全部 fq CLI 集成测试与 fq 库单元测试通过，
`cargo clippy -- -D warnings` 与 `cargo fmt --check` clean。经多轮纵深复审收敛，
未再发现新问题。
