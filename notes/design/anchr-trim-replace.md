...
1. **kmask `maskfullycovered` 命中语义反了**：bbduk 命中 kmer 窗口
   **不操作**（保持已置位），只 clear 未命中窗口；pgr 误在命中时也
   clear → 最终只留"不在任何扫描窗口内"的碱基。Lambda 数据 adapter
   命中稀疏、内部位置都被未命中窗口覆盖，巧合一致；连续 adapter 匹配
   反例暴露。修复 3 处（正常 kmer + 短 kmer 左右端）。
2. **`testOptimal` 的 avgErrorRate 缺 bbduk 0/1 特例**：`phredToProbError`：
   q<=0→0.75、q<=1→0.7，pgr 用 `10^(-0.1q)` → trimq=0/1 时 Kadane 的
   N 碱基 delta 符号不同（bbduk nprob=0.825 使 delta<0 剪尾、pgr
   nprob=1.0 使 delta=0 保留）。changequality 把 ACGT 质量提到 ≥2 掩盖了
   纯质量场景，末端 N 反例暴露。
3. **forceTrim 的 `ftm` 短 read 下溢 panic**：`len-1-len%ftm` 在
   len<ftm 时为负，Java int 允许、Rust usize 下溢 panic（bbduk 输出
   1bp）。修复：b0/b1/b2 用 i64 计算、right 取 `max(0)`。

其余复查点均与 bbduk 一致：`symbolToNumber0` 对 N 返回 0（baseToNumber0
fill 0）、GC 的 N 不计数、maq 的 `expectedErrors`（N 不加概率但分母含
N）、polymer `detectPolyLeft/Right` 的 nonpoly 容错、pairs.rs 交错/双文件
尾 read 处理、`fmt/seq.rs` 质量长度校验、changequality（N→0/ACGT→min2）。
新增 3 个合成回归测试锁定上述 bug。

---

## 7. trim-qual：sickle 环节（并入自原 fq-trim-qual.md）

### 7.0. 需求来源：anchr 中 sickle 的真实用法
...
### 7.2.2 滑窗核心（移植自 sickle `sliding_window`）
...
### 7.2.3 Mott 核心（移植自 cutadapt `quality_trim_index`）
...
### 7.2.4 双端 + singles（对齐 sickle pe 语义）
```
对每对 (R1, R2)：
  两端都通过 → 写 R1 输出、R2 输出
  仅一端通过 → 通过端写 singles
...
### 7.6. 参考对照：BBTools BBDuk（2026-08 补充）
