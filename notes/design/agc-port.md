...
- [x] `libs/fmt/twobit.rs`: `read_u32` / `read_u64` / `read_u32_vec` 改为 `pub`（原计划供 pbit
  复用；实际实现中 pbit 为避免冗余 `is_swapped` 参数，在 `format.rs` 内定义了独立的
  `read_u32_le` / `read_u64_le` 等，仅记录级 `read_2bit_record` / `write_2bit_record` 被复用；
  见 §复用-2.5）
...
**31 个集成测试**，全部通过（运行 `cargo test --test cli_pbit` 验证）。测试数据采用
...
- [x] 额外：`test_pbit_custom_min_match_len_roundtrip`（自定义 `-l` 回归测试：min_match_len 写入
  header 并被解压端正确使用）
...
- 54 个单元测试 + 31 个集成测试全部通过；性能基准 `benches/pbit_benchmark.rs` 编译通过
