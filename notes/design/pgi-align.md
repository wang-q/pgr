...
> 优化（输出逐字节等价），并对照源码裁定 Gap_Improver 不移植（§3.1）。
>
...
- **内部优化**（2026-08-05）：`dist pgi`/`stat`/`to-hv` 复用 `PgiMmap`
  （§3.2）；种子合并 lcp 连续传播（§3.3）——两者输出均逐字节等价。
...
批内顺序维护前驱。语义不变（plen ∈ [12, k] 的选择逻辑不动）。

**结论：落地**（2026-08-05）。输出逐字节等价（734 块 / 4,951,926 aligned）；
A/B 实测 merge 107 vs 109 ms（~2%）。E. coli 规模收益有限（瓶颈在磁盘
IO），但零风险、是 §7.3 变长种子的前置机制。这是 FastGA 源码语义的直接
对应。
...
**结论**：完整 LCP 是"与 FastGA 实现形态对齐"的架构改进——直接收益在人类
规模（推测），附带 `.pgi` v3 格式演进与变长种子前置；E. coli 阶段收益小
但零风险。2026-08-05 起尝试移植（见 git 历史与下文落地记录）。

**尝试结果（2026-08-05 实测，否决 merge 侧改造）**：把窗口扫描改为
FastGA 式"逐碱基递增收缩"（`window(m+1)` 空即最大 m）做性能上限实验：

- **merge 543 ms vs 简化版 107 ms（慢 5×）**——每步 `window(m+1)` 都是
  PgiQuery 二分（O(log n)），250 万条目 × ~18 步 = 数千万次二分；FastGA
  的 O(1) 依赖 C 指针推进，PgiQuery 抽象没有该优势；
- 且忽略频率过滤时丢种子（715 vs 734 块）——`window` 非空不能区分高频
  条目（FastGA 的 GIX 构建时已滤除高频 k-mer，pgr 在 merge 侧过滤），
  修复需遍历窗口，即回到当前实现。

**结论**：完整 LCP 的 merge 侧改造在 pgr 架构下**不可行且有害**；当前
简化版（lcp 起步 + 窗口扫描 + 跳过高频 `shared_prefix`）已是合理形态。
`.pgi` v3 存 LBYTE 属独立架构演进（非性能优化），暂缓。
