...
- 10 株 cohort 验证见 [[design/hv.md]]（引用
  §2.4 的身份率矩阵）。
...
#### 7.3.1 立项评估实验（2026-08-12，结论：不做）

用户提出"变长种子在差异较大区域捕获同源"的论点后，用 E. coli（mg1655 ×
sakai）与酵母（S288c × RM11_1a）实测三种种子（固定 k=40 / 固定 k=12 /
变长 [12,∞) 的最长精确匹配）：

| 指标 | E. coli | 酵母 |
| :--- | :---: | :---: |
| 12-mer 位置命中率（短端存在性） | 85.6% | 84.4% |
| 40-mer 位置命中率（当前种子） | 54.5% | 49.9% |
| 64-mer / 100-mer 位置命中率（长端存在性） | 43.3% / 32.0% | 45.8% / 41.0% |
| `align pgi -k 12` 端到端覆盖增量 | **+6,334 bp（0.15%）** | **+1,032 bp（0.009%）** |
| `-k 12` 总覆盖变化 | −832 bp（略降） | −9,307 bp（略降） |

**结论**：

* **短端（12–40 bp 精确匹配）在"位置命中"层面确实存在**（40-mer 未命中的
  位置有 31%（E. coli）/ 34.6%（酵母）被 12-mer 命中），**但端到端 PSL 覆盖
  几乎无增量**（≤0.15%）——pgr 的链化 + 波/带状扩展已覆盖这些区域，短种子
  不改变最终比对（"位置命中 ≠ 比对收益"）；
* **长端（>40 bp 延伸）全部落在 40-mer 已命中的位置**（64/100-mer 命中 ⊂
  40-mer 命中），对发现同源零增量，只影响链化连续性/打分（pgr 链化已覆盖）；
* **降 k 有副作用**：k=12 覆盖反而略降（短种子噪声 + 频率过滤交互）；
* 与 benchmark 一致：pgr 与 FastGA 的剩余差距（+2.4%）来自 wave 补齐，
  不是种子端。

**判定：完整 adaptamer（变长种子 >k）不做**——位置命中的增量已被链化/
波扩展吸收，端到端无收益；若未来人类规模数据显示新场景再评估（§7.2 决策门）。

#### 7.3.2 性能维度补充实验（2026-08-12，极长相似片段）

用户补充"变长种子在极长相似片段会加速"的论点后，补测性能维度（自比对 =
极长相似片段的最极端情形）：

**端到端耗时（S288c × S288c 自比对，12 Mb）**：

| 实现 | 耗时 |
| :--- | :---: |
| pgr align pgi（固定 40-mer） | ~2.7 s |
| FastGA（变长种子） | **1.29 s** |

pgr 耗时分解（RUST_LOG=debug）：pgi build ×2 ≈ 700 ms、merge（种子）
300 ms、chain_tubes 246 ms、**extend（波扩展/精化）1213 ms**。

**关键数据**：

* **40-mer 出现次数分布**（自比对视角，单链）：酵母 94.8% 唯一（1 次）、
  E. coli 97.9% 唯一；变长种子把"多重命中降为 1"最多省 3.86%（酵母）/
  1.42%（E. coli）的 seed hits——**绝大多数种子本就唯一，变长种子减少
  空间极小**；
* **`--freq` 敏感性**（模拟种子特异度）：hits 377 万（freq=10）→ 500 万
  （freq=1000），merge 耗时几乎不变（281→319 ms，顺序推进已高效），
  extend 变慢（1290→1685 ms）——种子数量对 extend 有影响，但变长种子
  只能省 ~4% hits，对应总耗时预计 ≤3%（~50–80 ms）；
* 瓶颈是 extend（45%）与索引构建（26%），不在种子端。

**结论（补充）**：FastGA 在自比对更快（1.29 vs 2.7 s）的来源是其 C 实现
效率（FAtoGDB 索引构建 + 波扩展），**不是变长种子本身**——种子绝大多数
已唯一，变长种子可减少的 hits ≤4%，对 pgr 总耗时加速 ≤3%。**性能维度
同样不支持立项**；若追 FastGA 的剩余差距，方向是波扩展/索引构建的实现
效率，不是种子机制。
...
- **触发条件（先量化再实现）**：
  1. 45 对 cohort 抽 5–10 对跑 (A,B) vs (B,A)，对比覆盖/身份/块数——
     差异在噪声级则全对场景也不需要；**已量化（2026-08-12，带序列
     路径）**：mg1655×sakai +0.49%、mg1655×se11 −1.81%、sakai×se11
     +0.51%、mg1655×cft073 +0.08%（aligned 覆盖差，A→B vs B→A），
     多数 <1%——接近噪声级，`--sym` 默认关保持不变。⚠️ 量化必须走
     `--ref-seq/--query-seq` 路径：不带序列的输出是未精化的
     geometric blocks（每个种子链管一个粗 block，含链内 gap，低身份度
     是设计使然），不能用于比对质量/方向偏差评估（2026-08-12 误用后
     重测）。
