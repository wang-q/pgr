...
4. **真实验证**：`scripts/verify-hybrid-real.sh` 拆分适配为 fill/rest 两路，
   ...
   2026-08-06 release 实测）：

   | 引擎 | 耗时 | PSL 记录 | MAF 块 | 覆盖 |
   |---|---|---:|---:|---:|
   | pgi-only | 1.2 s | 738 | 582 | 90.74% |
   | fill | 13.8 s | 3296 | 576 | 91.85% |
   | rest（默认 syncmer 预筛） | 6.5 s | 1764 | 553 | 91.81% |
   | rest（--sampler none 全量） | 23.7 s | 2005 | 557 | 92.20% |
   | fill + rest（默认） | ~18 s | 5060 | 605 | 92.10% |
   | lastz-only | 134.0 s | 21793 | 382 | 93.11% |

   结论：fill 快（比 lastz 快 ~12×）补锚点间 gap；rest 默认采样预筛
   6.5 s（比全量 23.7 s 快 3.7×，覆盖 -0.39 pp），组合覆盖 92.10%；
   双侧 holes 裁剪的固有盲区造成与 lastz 的 ~1.0 pp 差距，属设计取舍。
   模拟灵敏度：rest 255/600 ≈ lastz 256/600（cell diff 1），假阳性
   0.409% < 1%。

**覆盖口径修正（2026-08-06 用户指出）**：上述表为原始 PSL 并集口径；以
`chainnet --syn` 输出的 MAF（最终 syntenic 结果）为准重测：

| 引擎 | PSL 并集 | MAF（chainnet --syn） |
|---|---:|---:|
| pgi-only | 90.74% | 89.297% |
| fill | 91.85% | **89.991%** |
| rest（预筛默认） | 91.81% | 89.985% |
| fill + rest 合并 | 92.10% | **89.866%**（低于 fill/rest 单独） |
| lastz-only | 93.11% | 90.237% |
...
| rest（197 holes × query holes） | 132.6 s | 22.1 s | 13.8 s | 13.5 s |
