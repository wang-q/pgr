...
| `-f/--freq` | 高频 k-mer 跳过阈值 | 10（默认）→ 100 | **放宽** | 高拷贝家族（Alu 百万拷贝）种子会超频被丢；**2026-08-12 定稿 100** |
| `--min-shared` | 共享种子最小长度 | 12（tube 默认）→ 16 | 放宽 | 高分歧拷贝共享种子少；**2026-08-12 定稿 16** |
| `-p` | 线程数 | 8 | — | |
...
### 2.5.1 真核标定结果（2026-08-12，S288c + repbase）

**方法**：S288c（12.16 Mb，先 `tr a-z A-Z` 去 soft-mask）+
repbase 清洗库（`pgr fa filter --upper --iupac --dash --uniq`，31,491 条）；
参考 = 本仓库 `pgr rept masker`（RM 4.2.4 `-lib` 复刻，3m49s），再减去
TRF 简单重复部分、切掉 <50 bp 碎片得到"库命中 strict 参考"
（592,899 bp / 4.88%）；全矩阵 recall 用 `pgr runlist statop` 计算。

**参数扫描结果**（k31/s8/w5、min-identity 0.70、min-len 50、fill 10 固定；
recall = e-align 覆盖 ∩ 参考 / 参考；over-mask = e-align 独有 bp 占基因组比）：

| 配置 | 覆盖 bp / % | recall | over-mask |
| :--- | :--- | :--- | :--- |
| f10 / ms12 | 374,317 / 3.08% | 62.8% | 0.015% |
| f10 / ms16 | 368,266 / 3.03% | 61.8% | 0.013% |
| f50 / ms12 | 403,247 / 3.32% | 67.4% | 0.029% |
| f50 / ms16 | 399,759 / 3.29% | 67.0% | 0.022% |
| **f100 / ms16（定稿）** | **404,293 / 3.33%** | **67.6%** | **0.029%** |
| f100 / ms12 | 407,008 / 3.35% | 67.9% | 0.037% |
| f500 / ms16 | 405,224 / 3.33% | 67.6% | 0.036% |
| k40 / f100 / ms16 | 403,801 / 3.32% | 67.5% | 0.029% |
| k21 / f100 / ms16 | 402,502 / 3.31% | 67.3% | 0.030% |

**结论（已落地 e-align 默认值 f100/ms16）**：

*   **freq 10 → 100 验证成立**：f10 明显低敏感（61.8–62.8%），f50 居中
    （67.0–67.4%），f100 与 f500 持平（67.6%）——100 是敏感度/噪声平衡点；
*   **min-shared 16 优于 12**：recall 仅 -0.3pp，但 over-mask 降 22%
    （0.037→0.029%）；细菌上 12 更是过度敏感（MG1655 1.38% vs 参考
    1.06%），16 落回 1.29%——两端一致支持 16；
*   **k31/k40/k21 等效**（67.3–67.6%），保持 k31 默认；
*   **min-identity 0.60 与 0.70 输出逐字节相同**——pgi 链延伸产出的 block
    天然 ≥70% identity，该参数在真核上不敏感（与 §2.3.5 细菌结论一致）；
*   **recall 天花板 ~68% 是种子敏感度边界而非参数问题**：漏检 192 kb 中
    69% 为 <100 bp 碎片（median 75 bp，max 1.3 kb），对 repbase 回比对
    为 83–92% identity 的真实 Ty LTR 片段（TY/TY1-LTR/TY5-LTR 等）——
    k31 syncmer 种子够不到短分歧拷贝（RM 用 word_size 9 + 225 minscore，
    无种子长度门槛）；`--min-len 30` 与 50 输出相同，排除长度过滤假象。
    MITO（27 kb，GC 11.5%）是 AT 富集区被 repbase 高遮蔽的参考侧
    over-masking（参考遮了 MITO 32%），e-align 不追。

**对照（MG1655 + tncentral，2026-08-12）**：旧默认 f50/ms12 = 64,022 bp
（1.38%），新默认 f100/ms16 = 59,969 bp（1.29%），RM 参考 1.06%——细菌
上同样更干净，无回归。
