...
- [快速参考](#快速参考)
- [典型场景](#典型场景)
- [pgr 现有基础设施复用](#pgr-现有基础设施复用)
- [Rust 架构建议](#rust-架构建议)
- [文件格式规范](#文件格式规范)
- [PAF 驱动的 CIGAR delta 编码](#paf-驱动的-cigar-delta-编码)
- [决策记录](#决策记录)
- [参考资料](#参考资料)
...
| 子命令 | 分组 | 用途 | 关键参数 |
|--------|------|------|----------|
| `create` | build | 从参考 FASTA 和样本 FASTA 创建归档 | `-r ref.fa`, `-i sample.fa`, `-o out.pbit` |
| `append` | build | 向已有归档追加样本 | `in.pbit`, `-i sample.fa`, `-o out.pbit`（可选） |
| `to-fa` | transform | 提取所有样本为 FASTA | `in.pbit`, `-o out_dir/` |
| `some` | subset | 按样本名列表提取 | `in.pbit`, `sample_list.txt`, `-o out.fa` |
| `range` | subset | 按 contig/区间提取所有样本 | `in.pbit`, `chr1:1-1000`, `-o out.fa` |
...
1. **高质量参考 + 同源样本（无复杂 SV）**
   ...
   ```
   样本与参考 contig 名一致、结构简单，LZ-diff 即可得到较好压缩率。
...
2. **含结构变异/重排的多样性样本**
   ...
   ```
   PAF 驱动的 CIGAR 模式能处理 indel、重排、跨 contig 比对。CIGAR 标签建议用 `--eqx`，
   避免 pbit 在压缩时做 `M` → `=/X` 拆分。

3. **多样本分批追加（混合模式）**
   ```bash
   pgr pbit create -r ref.fa --name samples.tsv -o cohort.pbit
   pgr pbit append cohort.pbit --name more_samples.tsv -o cohort2.pbit
   ```
   `samples.tsv` 支持三列 `sample_name<TAB>fasta_path<TAB>paf_path`（第三列缺失则走 LZ-diff）。
...
pub struct DeltaEntry {
    ...
    pub raw_length: u32,
    pub packed_data: Vec<u8>,       // flate2(LZ-diff / CIGAR encoded)
...
pub struct SegmentDesc {
    ...
    pub ref_start: u32,       // segment-relative start in reference 2bit record (CIGAR only; 0 for LZ-diff)
    pub ref_end: u32,         // segment-relative end in reference 2bit record (CIGAR only; 0 for LZ-diff)
    // is_rev_comp / raw_length / encoding live in DeltaEntry (properties of the
    // encoding, shared by all segments pointing to the same delta).
}
...
pub struct Decompressor<R: Read + Seek> {
    ...
    // Delta metadata loaded during `new` by scanning Delta Data headers (10 bytes each).
    // Stores is_rev_comp / raw_length / packed_size / encoding per delta so that
    // `get_contig` can compute segment coordinates and pick decode path without
    // re-reading headers from disk.
    delta_meta: Vec<Vec<DeltaMeta>>,            // [ref_group_id][delta_id] → (is_rc, raw_len, packed_size, encoding)
    delta_offsets: Vec<Vec<u64>>,              // delta_offsets[ref_group_id][delta_id] → file offset of header
    delta_cache: LruCache<(u32, u32, u32, u32), Vec<u8>>,  // (ref_group_id, delta_id, ref_start, ref_end) → decoded raw seq
}
...
impl<R: Read + Seek> Decompressor<R> {
    ...
    /// `contig_groups`, `contig_set`, and scans Delta Data headers (10 bytes each)
...
│   ref_group 0: delta 0, delta 1,... │     (is_rc + raw_len + packed_size + encoding +
│   ref_group 1: delta 0, delta 1,... │      flate2(LZ-diff / CIGAR encoded))
...
for each ref_group:
  ...
  for each delta:
    ...
    1      encoding      0 = LZ-diff, 1 = CIGAR
    packed_size packed_data  flate2(LZ-diff / CIGAR encoded bytes)
```
...
> 扫描仅需读取每条 delta 的 10 字节头部（`is_rev_comp + raw_length + packed_size + encoding`），不解压数据。
...
for each sample:
  ...
  for each contig:
    ...
    for each segment:
      ...
      4      ref_start       参考段内相对起始（CIGAR 段用；LZ-diff 段填 0）
      4      ref_end         参考段内相对结束（CIGAR 段用；LZ-diff 段填 0）
      (is_rev_comp / raw_length / encoding 由 DeltaEntry 提供，此处不重复)
4      cmd_line_len
...
> **多参考与内嵌参考索引段扩展**（泛基因组增量设计草案）已独立成文：
> [[pbit-index-extension.md]]。
...
| 方案 | 核心思想 | 优点 | 缺点 |
|------|----------|------|------|
| A：PAF 仅指导参考选择 | PAF 用于选参考段和方向，delta 仍用 LZ-diff | 改动最小，格式不变 | 不保留比对信息；indel 处 LZ-diff 效率差 |
| B：PAF 指导 + 存储比对元数据 | 在 Collection 中存比对摘要，delta 仍用 LZ-diff | 保留比对关系 | 仍受 LZ-diff indel 效率限制；元数据冗余 |
| C：CIGAR 替代 LZ-diff | 用 PAF CIGAR 直接作为 delta 编码 | 天然处理 indel/重排；省 LZ-diff 计算；保留比对 | 依赖外部比对工具；需处理未覆盖区域；格式需扩展 |
| D：混合编码 | 每段独立选择 LZ-diff 或 CIGAR | 兼顾两种场景；未覆盖区域不丢数据 | 解压端需支持两条路径；复杂度略增 |

技术细节（`encoding` 字段、`packed_data` 格式、混合模式回退策略）见 §5、§7、§8。
...
pbit 使用**两个**结构体共享同一在盘头部：
- `DeltaMeta`（10 字节，仅头部）—— `Decompressor::new` 扫描所有 delta 头部构建
...
`encoding` 字段位于 `DeltaMeta`：`get_contig` 决定走 LZ-diff 还是 CIGAR 解码路径时只看
`delta_meta`（不读 packed_data），从而保留随机访问能力。
...
pub struct DeltaMeta {
    ...
    pub raw_length: u32,
    pub packed_size: u32,
    pub encoding: DeltaEncoding,  // 在盘第 10 字节
}
...
pub struct DeltaEntry {
    ...
    pub encoding: DeltaEncoding,  // meta() 自动带上
}
...
1       4     raw_length
5       4     packed_size
9       1     encoding        // 0 = LZ-diff, 1 = CIGAR
...
- PAF 必须包含 `cg:Z:` CIGAR 标签。minimap2 默认输出 CIGAR；若缺失，该 PAF 记录视为未覆盖。
- 推荐用 `--eqx` 让 minimap2 直接区分 `=/X`，避免 pbit 在压缩时做 `M` → `=/X` 拆分。
- `-i` 与 `-p` 必须一一对应，数量不等时报错。
- `--name` TSV 与 `-p` 互斥；多样本场景建议用 `--name` 三列模式，避免顺序歧义。
- 一段样本序列若未被 PAF 完整覆盖，自动回退 LZ-diff，不会报错。
...
扩展为**可选三列** `sample_name<TAB>fasta_path<TAB>paf_path`：

完整文件示例（制表符分隔）：

```tsv
# samples.tsv
sample1	/data/sample1.fa	/data/sample1.paf
sample2	/data/sample2.fa
sample3	/data/sample3.fa	/data/sample3.paf
```

- 第二列：样本 FASTA 路径
- 第三列（可选）：PAF 路径；缺失时该样本走 LZ-diff
...
PAF `-` 链的 CIGAR 描述的是 `RC(query)` vs `forward(target)`。因此压缩端需先完成坐标转换，
确保截取出的 CIGAR 与样本序列方向一致：

1. **坐标映射**：用 `forward_to_rc_coords` 将样本段正向坐标 `[seg_start, seg_end)` 映射为
   RC 坐标 `[query_end - seg_end, query_end - seg_start)`，再调用 `slice_cigar_by_query`
   （此时 CIGAR 起点对应 RC(query) 位置 0）。
2. **样本切片**：截取后得到的 `sample_slice` 必须是 `rev_comp(seg)`，确保 X/I 碱基与 CIGAR
   描述的比对方向一致。
3. **解压还原**：`apply_cigar` 先正向应用 CIGAR 得到 `RC(sample)`，再按 `is_rev_comp` 做
   反向互补，得到原始样本段。

相关实现：`compressor.rs:90-636`、`decompressor.rs:270-349`。
...
**未覆盖区域处理**：PAF 没覆盖的样本段统一**回退 LZ-diff**（对到同名参考段，encoding = LZ-diff）。
直接存原始序列（encoding = Raw）需引入第三种编码，当前不采用。
...
> **`decode_delta` 签名**：`decode_delta(&mut self, seg: &SegmentDesc)`
> （[decompressor.rs:270](file:///Volumes/ExtHome/Scripts/pgr/src/libs/pbit/decompressor.rs#L270)）
> 接收完整的段描述。CIGAR 模式下步骤 2c 使用 `seg.ref_start`/`seg.ref_end` 切片参考段；
> LZ-diff 路径忽略这两个字段。缓存键 `(ref_group_id, delta_id, ref_start, ref_end)` 包含
> 后两者，因为同一 delta 在不同参考区间会解码出不同序列。
...
> （`A=0,C=1,G=2,T=3,N=4`，需 3 bit）。实现上每碱基存 1 字节（ASCII，简单），或打包为 3-bit 流（紧凑但复杂）。
...
>
> #### 实测（2026-08-02，E. coli 真实数据）
>
> 参考 = MG1655（NC_000913，4.64 Mb）。样本 contig 名必须与参考一致（AGC 风格约束，
> 两模式都按名匹配；实测把 Sakai/SE11 的 contig 重命名为 NC_000913 后测试）：
>
> | 模式 | 样本 | 归档大小 | 往返 |
> |------|------|----------|------|
> | LZ-diff | Sakai（chr + 2 质粒）+ SE11（chr + 6 质粒）| 4.69 MB | 无损（去换行后逐字节一致）|
> | CIGAR（FastGA -pafx PAF）| Sakai 染色体 | 3.00 MB | 无损 |
>
> `to-fa` 全量解压与 `range` 随机访问（`NC_000913:100000-100100`）结果均与输入精确一致。
> 注意：样本内多 contig 重命名为同一名字时 `to-fa` 会合并为一条记录（名字需唯一）。
> Phase 8f 的完整基准（PAF 连续性 vs CIGAR 命中率维度）仍未实施。
>
...
PAF 驱动的 CIGAR delta 编码已在 8a–8e 实现并测试通过；8f 为可选基准，尚未实施。

#### Phase 8a: CIGAR 编解码基础设施 — 已实现
...
- `decode_delta(&mut self, seg: &SegmentDesc)`（CIGAR 段使用 ref_start/ref_end，见 §8 说明）
- 复用 `libs/paf/msa_build.rs` 的 CIGAR 应用逻辑

#### Phase 8e: CLI 集成 — 已实现
- `create.rs` / `append.rs`：支持 `--paf` / `-p` 参数；`--name` TSV 扩展为可选三列（见 §6.3）
- 已覆盖的测试场景（分类）：
  - **基本往返**：`=/X/I/D` CIGAR（`+` 链）、`-` 链 RC 语义、纯 LZ-diff、CIGAR + LZ-diff 混合模式。
  - **边界回退**：未覆盖段、CIGAR target 投影跨参考段边界、PAF 无 CIGAR 标签、空 PAF 文件。
  - **复杂比对**：重排场景、`M` 操作拆分、`--name` 三列 TSV 混用。
  - **输入校验**：`--paf` 与 `--name` 互斥、`-i`/`--paf` 数量不匹配、单条 PAF 解析错误跳过并 warn。
...
4. **`M` 操作处理**：`pgr maf to-paf` 已改为在源头区分 `=/X`（[cigar.rs](file:///Volumes/ExtHome/Scripts/pgr/src/libs/paf/cigar.rs)
   ...
   **注意**：`M` → `=/X` 拆分需读取样本序列（比对 M 区域的样本碱基与参考碱基）。压缩端有样本
   ...
   minimap2 `--eqx`）无此开销。实现见 `compressor.rs` 的 `split_m_to_eqx` 函数。

5. **query-side 索引方案**：pbit 自建 query-side 区间树，不复用 `PafIndex.reverse_trees`。
...
6. **PAF 无 CIGAR 处理**：若 PAF 记录无 `cg:Z:` CIGAR 标签（`extract_cigar` 返回空 Vec），
   该记录不插入 query-side 索引（视为未覆盖，相关样本段回退 LZ-diff）。若整个 PAF 文件无
   任何 CIGAR，所有样本段回退 LZ-diff（等价于无 PAF 输入）。实现时不报错，仅 log 警告。

7. **PAF 解析错误处理**：单条 PAF 记录解析错误（格式错误、坐标非法、字段缺失等）时跳过
   该记录并 `log::warn`，相关样本段视为未覆盖，回退 LZ-diff（与决策 6 一致）。若整个 PAF
   文件无法打开或非 PAF 格式（如文件不存在、首行解析失败且无有效记录），返回错误终止压缩
   （`anyhow::bail!`），避免用户误以为 PAF 已生效但实际全部回退。

8. **内嵌参考索引 vs 独立索引文件**：泛基因组增量场景下，**参考索引内嵌 pbit**
   （与参考 2bit 记录成对追加），**样本索引临时即弃**。理由：
   (a) 泛基因组以图（GFA）为最终产物，pbit/索引均为中间产物、可重建丢弃，格式演化
   无长期兼容负担；
   (b) 内嵌单文件带来原子管理与分发简洁（一个 pbit 即参考序列 + 参考索引 + 样本 delta）；
   (c) 内嵌量 = 参考数（几十），不随样本数（4 万）增长，避免文件膨胀；
   (d) pbit 的 append-only 与"参考段 + 索引段成对追加"天然契合（旧段不动，尾部追加 +
   重写 Footer）。详见 [[pbit-index-extension.md]]。
...
- pbit 库入口: [src/libs/pbit/mod.rs](file:///Volumes/ExtHome/Scripts/pgr/src/libs/pbit/mod.rs)
- 文件格式与二进制 I/O: [src/libs/pbit/format.rs](file:///Volumes/ExtHome/Scripts/pgr/src/libs/pbit/format.rs)
- LZ-diff 算法: [src/libs/pbit/lz_diff.rs](file:///Volumes/ExtHome/Scripts/pgr/src/libs/pbit/lz_diff.rs)
- CIGAR delta 编解码: [src/libs/pbit/cigar_delta.rs](file:///Volumes/ExtHome/Scripts/pgr/src/libs/pbit/cigar_delta.rs)
- PAF query-side 索引: [src/libs/pbit/paf_index.rs](file:///Volumes/ExtHome/Scripts/pgr/src/libs/pbit/paf_index.rs)
- 压缩端: [src/libs/pbit/compressor.rs](file:///Volumes/ExtHome/Scripts/pgr/src/libs/pbit/compressor.rs)
- 解压端: [src/libs/pbit/decompressor.rs](file:///Volumes/ExtHome/Scripts/pgr/src/libs/pbit/decompressor.rs)
- 样本/参考组元数据: [src/libs/pbit/collection.rs](file:///Volumes/ExtHome/Scripts/pgr/src/libs/pbit/collection.rs)
- CLI 实现: [src/cmd_pgr/pbit/](file:///Volumes/ExtHome/Scripts/pgr/src/cmd_pgr/pbit/)
- CLI 集成测试: [tests/cli_pbit_paf.rs](file:///Volumes/ExtHome/Scripts/pgr/tests/cli_pbit_paf.rs)
