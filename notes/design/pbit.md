...
- **输出确定性确认（2026-08-09 排查结论）**：pbit 输出是确定性的（同参数
  同 `-o` 两次运行 md5 一致）。此前"两次不同"的虚惊源于 collection 元数据
  嵌入**完整命令行（含 `-o` 文件名）**（`collection.rs` 的 `cmd_line`
  字段，设计特性）——测试时每次换输出文件名导致字节差。序列化用有序
  Vec、gzip mtime=0，无 HashMap 顺序依赖。
