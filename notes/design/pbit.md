...
4. **PAF 存储基础**：PAF 只存坐标 + 匹配统计 + 标签（不存序列）；
   `qlen/tlen/qname/tname` 不重复存——由归档 contig 表恢复（v1009）。
...
> 状态：**定义与判定细节已定稿（2026-08-09），进入实现**。本节先记录
> v1009 的现状（含失误），再给出定稿的判定细节。
...
- **碎链**：**被其他大链覆盖的链**——即使其相似度更高，只要落在主链
  ...
  **碎链仍可编码其覆盖的段**（2026-08-09，2026-08-09 确认）——主/碎分类只
...
CIGAR 短，只是次级收益）；**更优先保证匹配在 sample 上连续**：

- "连续"是**单条链自身**的性质：query 区间连续、跨的段完整（段边界不
  中断、段与段之间无缺口）；
- 连续链的优势（压缩机制）：段与段自然衔接（`q_start`/`ref_start` 线性
  推进）、`to-paf` 重建无断裂、LZ/Raw 兜底最少、`=` 段长可合并 → CIGAR
  文本短；
- 相似度（CIGAR 短）降级为**同覆盖 / 同连续下的次级 tiebreak**。
...
3. **pgi 长链链化**（**挂账**，依赖对齐器）：minimap2 式 chaining 产出
   ...
   （`psl to-paf` 无 cg:Z；链级 `cg:Z` 生产者**明确不做**，2026-08-09
   用户裁定——推荐链路 `chainnet → maf to-paf` 自带 cg:Z。）
