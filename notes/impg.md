...
是**局部按需**物化——每次查询从一个区间出发。pgr V4a 走 seqwish 路线（粗全局 GFA），V4b 走 impg 路线
（区域精细 GFA），同时利用两种粒度。
...
- **图构建生态（6 个，pangenome 团队）**
    ...
    - **seqwish**（pangenome/seqwish）— 从 PAF 诱导 GFA。 入口：
      ...
      V4a 完全适用，pgr 输入 PAF 与 seqwish 一致。
...
pgr V1 已借鉴此设计 落地 `reverse_trees` 双向索引（仅 `+` 链建 mirror，负链不建；见
...
`M` 保持不变。这是 pgr mirror index 设计可借鉴的细节——pgr V1 的 `reverse_trees`仅 `+` 链建 mirror
...
> - ✅ V1 已实现：`pgr paf query -o bed`（可选，PAF 默认）+ `-b regions.bed` 批查
...
> V1 已补 BED（`-o bed` 可选）与批查（`-b`），**默认输出保持 PAF**（与 impg 的 BED 默认不同，
> 理由见 [[paf-pangenome.md]] §3.1）。**V4a 已实现**`pgr paf graph -f refs.fa --min-var-len 100`，
> 输出 GFA v1.0（S/L/P），seqwish DSU 风格（详见 [[paf-pangenome.md]] §4.3.1、[[seqwish.md]]
> §6.2）。下一步：V2 补 fasta，V3 补 maf，V4b 区域精细 GFA（impg 风格），V5 补 MAF/VCF 输出。
