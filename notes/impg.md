...
**物化图内部的粒度差异**：seqwish（PGGB 路线）的物化是**全局一次性**的——一次性算出全部等价类再
写图（spanning tree → BFS discovery → DSU union-find，详见 [[seqwish.md]] §3）；impg 的 `query -o gfa`
是**局部按需**物化——每次查询从一个区间出发。pgr V4a 走 seqwish 路线（粗全局 GFA），V4b 走 impg
路线（区域精细 GFA），同时利用两种粒度。
...
- **图构建生态（6 个，pangenome 团队）**
    ...
    - **seqwish**（pangenome/seqwish）— 从 PAF 诱导 GFA。
      ...
      **pgr 已对 seqwish 源码做独立分析**（[[seqwish.md]]），其算法骨架（spanning tree → BFS discovery
      → DSU union-find → compact → links → GFA）对 pgr V4a 完全适用，pgr 输入 PAF 与 seqwish 一致。
...
**双向索引的 CIGAR 处理** — [`invert_cigar_ops`](file:///Volumes/ExtHome/Scripts/pgr/impg-0.4.1/src/impg.rs#L144)
是 bidirectional 索引（§3.1）的配套机制：生成 B→A mirror entry 时，需把 A 视角的 CIGAR
...
[`alignment_record.rs`](file:///Volumes/ExtHome/Scripts/pgr/impg-0.4.1/src/alignment_record.rs) 定义了
PAF/1ALN/TPA 三种比对格式的统一表示，是 §1.2 双后端架构的"格式抽象层"物理载体（§1.3 提及）。
...
- **PAF**：`strand_and_data_offset` 低位 = 文件偏移量（指向 `cg:Z:` 标签）；`data_bytes` = CIGAR 字符串字节数。
- **1ALN**：`strand_and_data_offset` 低位 = alignment index（O(1) seek）；`data_bytes` = tracepoint 数量。
- **TPA**：同 1ALN 的 tracepoint 编码。

**strand 编码** — `STRAND_BIT = 0x8000000000000000`（MSB），`strand()`/`set_strand()` 通过位运算读写，
不占用额外空间。这与 `QueryMetadata`（§3.2）的 `strand_and_data_offset` bit 63 = strand 设计一致——
`AlignmentRecord` 是入库前的中间表示，`QueryMetadata` 是入库后的区间树节点表示，两者共享 strand
编码方案。
...
- **`ResolutionConfig`** ([L35](file:///Volumes/ExtHome/Scripts/pgr/impg-0.4.1/src/resolution.rs#L35))
  是 crush 行为的完整参数化，按职责分六组（共 30+ 字段）：
    ...
    - **Auto 三档路由阈值** — `auto_spoa_max_traversal_len`/`auto_poasta_max_traversal_len`
      （median 阈值，设 0 禁用对应档）、`auto_allwave_max_total_sequence`/`auto_allwave_max_traversals`
      （legacy，仅 CLI 兼容保留）。
    - **bubble 规模守卫** — `max_bubble_span`（root path 最大跨度 bp）、
      `max_traversal_len`/`min_traversal_len`（单次直接替换的遍历长度上下界；
      `min` 允许显式分层 pass：先 crush 大 bubble 再跑小 bubble）、
      `max_median_traversal_len`/`max_total_sequence`/`max_traversals`（0 禁用对应守卫）。
    - **polish（§6.5.2 阶段 7）** — `polish_iterations`/`polish_max_traversal_len`/
      `polish_max_median_traversal_len`/`polish_max_total_sequence`/`polish_max_traversals`
      控制小 tangle POASTA 清理的规模预算，`polish_method`（`Poa`/`Poasta`/`Smooth` 三选一）。
    - **pairwise 引擎参数** — `abpoa_bin`（abPOA 可执行路径）、
      `pair_k_nearest`/`pair_k_farthest`（多序列 pairwise 引擎的近邻/远邻采样数）、
      `pair_trees`（独立 tree-sampling pass 数，取并集）。
...
- **`GfaEngine` 枚举**（5 变体）— §6.1 已详述。值得补充的是 `SyngNative` 的 doc comment
  完整描述了 6 阶段 syng-native 管道（per-partition 距离矩阵 → KNN pair selection →
  strand groom → anchor-seeded BiWFA → indel left-align → emit PAF → seqwish），是 syng
  后端免比对的算法骨架。`SyngLocal` 则是"取查询子集 → 重建局部 syng syncmer 图 → 同套
  blunt/crush/sort 变换"的快速通道。
...
- **`SeqIndexWrapper`**（私有）— 最小适配器，只实现 `seq_index()` 返回 `&SequenceIndex`，
  其余方法（`query`/`query_with_cache`/`query_transitive_*`/`get_or_load_tree` 等）全部返回
  空值。专供 `dispatch_gfa_engine_with_seq_index` 使用——当 query intervals 已由 syng 计算好、
  只需把 GFA 引擎接到 SequenceIndex 上时，避免构造完整 ImpgIndex 的开销。
...
- **`SyngImpgWrapper`**（pub）— 完整适配器，把 `SyngIndex` 包装成 `ImpgIndex`。核心是
  ...
  全部转发到 `SyngIndex::query_region`。**关键设计**：
    - **链过滤可选**（`with_chain_filter(min_anchors, min_fraction)`）— 启用时用 SweepGA
      plane-sweep 链式化 syncmer 命中，按 anchor 数 + query 覆盖率过滤弱链/paralog 噪声，
      再交给 partition 的 union-find；不启用时原始 per-syncmer 区间直通。
    - **`effective_min_chain_anchors_for_syncmer`** 自适应阈值 — 根据 query 长度与 syncmer
      长度动态调整最小 anchor 数，避免短 query 误过滤。
    - **`get_or_load_tree` 返回 `None`** — syng 无 alignment interval tree，调用方需走
      `query()` 路径而非直接读树。
    - **`syng_index_ref()` 提供 `Option<&SyngIndex>`** — 让需要完整 `HomologousInterval`
      （含 strand/genome name）的调用方绕过 `ImpgIndex` 抽象直接访问。

**关键认识**：`ImpgIndex` trait 抽象了"区间查询能力"，使 `partition`/`graph`/`genotype` 等
上层命令对"PAF 区间树 vs syng syncmer 索引"两种后端透明。**代价**是 trait 方法签名臃肿
（`query_transitive_bfs` 13 个参数），反映两种后端的最大公约数。pgr 若引入类似抽象，应优先
用更细粒度的 trait 组合（如 `IntervalIndex` + `TransitiveIndex` + `SequenceLookup`）而非
单一大 trait。
...
1. **公共类型汇聚点** — impg 把跨模块共享的类型（`GfaEngine`/`EngineOpts`/
   `SmoothPipelineConfig`）放在 lib.rs 而非独立 `config.rs`，是因为这些类型与引擎分发强耦合。
   pgr 的 `libs/paf/` 已有类似实践（`index.rs` 集中定义核心类型），可继续保持。
2. **双后端适配器模式的取舍** — impg 的 `ImpgIndex` 用一个胖 trait 抽象两种后端，签名臃肿。
   pgr 若未来要支持 PAF + Chain 双后端，应优先考虑 trait 组合而非单一大 trait。
3. **`partition_size: Option<usize>` 的开关式分区** — 同一个 `graph` 命令通过该字段切换
   "直接构建 vs 分区构建"，而非拆成两个命令。pgr 的 `pgr paf graph` 若未来需要分区模式，
   可借鉴这种"Option 字段触发分支"而非"独立子命令"的设计。
4. **`SyngImpgWrapper::with_chain_filter` 的可选过滤层** — syng 后端的链过滤是可选的
   （None 时直通原始区间），这种"过滤作为可选装饰器"模式对 pgr 的 `--syntenic-filter`
   设计有参考价值：Chain/Net syntenic 过滤可作为查询层的一个可选装饰器，而非强制路径。
...
- **`FeatureSpace`**（6 变体）— 候选 haplotype 的特征表示空间：
  `SyngSyncmerNode`/`GfaSegment`/`VariationGraphNode`/`LocalHaplotypeSegment`/
  `HaplotypeSequence`/`MemHit`。前两个对应隐式图（syng）与物化图（GFA）两条后端路径。
- **`EvidenceBackend`**（6 变体）— sample evidence 的投影后端：
  `Pack`/`Projection`/`SyncmerWalk`/`VariationGraphAlignment`/`HaplotypeAlignment`/`MemProjection`。
  `Pack` 对应 syng 的 pack 索引，`Projection` 对应区间投影（§4.1）。
- **`ScoringMethod`**（5 变体）— 评分方法：`Cos`（cosine 相似度，当前实现）、
  `CountLikelihood`/`AlignmentLikelihood`/`LearnedLocalEmission`/`GbwtImputation`（规划中）。

**cosine 评分实现** — 当前 `ScoringMethod::Cos` 把候选 haplotype 组合与 sample 都表示为
`FxHashMap<u32, u64>`（feature_id → count），用
[`score_cosine_combination`](file:///Volumes/ExtHome/Scripts/pgr/impg-0.4.1/src/genotyping.rs#L190)
计算 cosine 相似度，并转换为 QV：`qv = -10 * log10(1 - similarity)`（similarity ≥ 1.0 时 QV = 999）。
`CombinationScore` 返回 `similarity`/`qv`/`dot`/`sample_norm`/`genotype_norm` 五个量，供 ploidy
组合搜索排序。这种"feature 向量 + cosine"的评分模型是后端中立的数学核心——pgr 若做基因分型，
