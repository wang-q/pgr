...
工作量远超 100 行，且 V1 只需 MSA 不需 GFA。**结论：V1 不物化 GFA，推迟到 V3**。
...
**reference-guided 线性 MSA 比 POA 更快且无分支膨胀**。pgr 可在 `pgr paf msa` 中根据是否有
`--reference`参数选择后端：
...
[cal_cov.c](file:///Volumes/ExtHome/Scripts/pgr/minigraph-master/cal_cov.c) 的 `mg_cov_asm` 计算：
...
对 pgr V1（`pgr paf msa`）的影响：

- **不影响**：V1 继续走 PAF → POA → MSA 路线（[[graph-design.md §3]]），约 120 行新代码
