...
[[paf-pangenome.md §4.4]] 已记录 Caf 的 melting 过滤维度可作为传递闭包后处理。 minigraph 的 bubble 提供
**正交视角**：
...
**传递闭包置信度**的量化指标。这与 [[paf-pangenome.md §4.4]] 的 Degree 过滤对应。
...
两者维度不同，互不替代。详见 [[paf-pangenome.md]] §4.3.5。
...
对 pgr to-maf --msa（`pgr paf query -o maf`）的影响：

- **不影响**：to-maf --msa 继续走 PAF → POA → MSA 路线（[[paf-pangenome.md §3]]），约 150 行新代码
- **可借鉴**：`pgr paf query` 输出格式可参考 GAF 紧凑路径编码
- **可借鉴**：`pgr paf query --transitive` 的后处理过滤可参考 minigraph bubble 指标

对 pgr to-maf/to-maf --msa 的影响：
...
minigraph.md (本文档) ─ 架构参考 ──┐
    ...
    │                                           │
cactus.md ────────────── 架构参考 ─────────────┤
    │ §8 Caf 退火-熔化 → paf-pangenome.md §4.4     │
    │ §3 Minigraph-Cactus → paf-pangenome.md §4.5  │
    │                                           │
impg.md ──────────────── 路线参考 ─────────────┤
