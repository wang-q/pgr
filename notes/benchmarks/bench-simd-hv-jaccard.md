...
| n=10k, D=16384 | 4.63 ms | 8.45 ms | 5.98 ms | 8.62 ms | 17.4 ms | 26.9 ms |

AVX2 主路径实测：bit ±1 编码相对旧 wide bit **~5.9×**、相对旧 wide i8
**~3.8×**；i8 保语义 ~2.1×。AVX-512 参考列（同机测量）与 AVX2 持平，
佐证"256 作为主实现"的决策。速度来自两点：

> 2026-08-08 复测（同机，Ryzen 9 7945HX / stable 1.97）：上表全部数字
> 复现（差异均在噪声内），补充 D=16384 的 AVX2 现役实测
> （bit 4.63 ms / i8 8.45 ms，仍快于同维 AVX-512 参考）。
...
`hash_hv_bit_avx2` 逐指令一致的宏生成 6 个变体（仅 RNG 行不同，RNG 输出
统一 `black_box` 包裹以防编译器对常量 RNG 做 LICM 广播折叠），同机测量
（AVX2，n=10k/100k，D=4096）：

| RNG | n=10k | n=100k | vs 基线 |
|---|---:|---:|---:|
| **hash_hv_bit（现役基线，无 black_box）** | **1.1445 ms** | **11.445 ms** | 1.00× |
| rapid（宏版，同公式 + black_box） | 1.1650 ms | 11.756 ms | ~1.02× |
| wyrand（跳步，counter+mix） | 1.1682 ms | 11.627 ms | ~1.02× |
| 常量（RNG 免费，广播保留） | 1.1279 ms | 11.119 ms | ~0.99× |
| seed 原值不混合（仅测量） | 1.1272 ms | 11.265 ms | ~0.98× |
| splitmix64（跳步，counter+mix） | 1.5347 ms | 15.161 ms | ~1.33× |

解读：

* **广播 + 依赖链是绝对主体**：常量 RNG（计算完全免费）与基线只差 ~1.5%，
  mix 计算本身约占 2%——换任何 RNG 的收益上限都 <3%。
* **真实 RNG 候选全持平**：rapid / wyrand / 常量差异 <3%；splitmix64 反而
  慢 ~33%（3 次 64-bit 乘加混合比 rapid_mix 的 128-bit 乘法重）。
  换 RNG 无收益，维持原决策。
* **0.063 ms 是 LICM 假象**：不加 black_box 时，编译器把常量 RNG 的广播
  （`set1`）整体折叠/提升，测得 63 µs——不代表任何真实 RNG 可达。修正后
  常量与基线仅差 ~1.5%，进一步优化的真正方向是削减每 seed 广播/依赖链
  （非换 RNG）。
