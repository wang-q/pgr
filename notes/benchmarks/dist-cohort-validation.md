...
> 对照。日期：2026-08-02 初测、2026-08-04 修复轮后复测。
...
| 方法 | ρ（复测） | 备注 |
|---|---:|---|
| `dist seq`（k=8 草图，首 contig 对） | **0.792** | 略降：身份率矩阵随比对管线更新 |
| `dist pgi`（k=40 精确） | 0.542 | 与初测 0.539 一致 |
| `dist hv`（稀疏 v2，dim 4096） | 0.498 | 与 dist pgi 的 mash 排序一致（ρ=0.967，初测 0.9694） |

复测身份率矩阵用当前二进制重算（45 对，`--merge-gap 5000`）：块数整体
下降（如 mg1655–sakai 862 → 791，tube 链排序键修复 + syncmer 去重后
链发现变化），身份率分布几乎不变（0.9702–0.9960 vs 初测 0.9700–0.9962）。
...
> **更新（2026-08-02，复测 2026-08-04）**：`.hv`（稀疏）现在是 `dist pgi`
> 的 ~70× 快、0.97 排序一致的近似层；`dist seq` 仍是与身份率最贴近的
> 草图层（复测 0.79 vs 0.54），因为 k=40 syncmer 集合受采样位置漂移限制。
