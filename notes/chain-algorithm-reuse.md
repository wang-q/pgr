...
| Top-K 成分纯度检测 | `src/libs/ds/top_k_purity.rs` | 多类别计数中前 K 类占比过高时惩罚 | 无（纯计数） |
...
- `BitMap`、`GapCalc`、`KdTree`、`DupeTree`、`TopKPurity` 已下沉到 `src/libs/ds`，可由其他模块通过 `crate::libs::ds::*` 使用。`chain` 模块仍通过 `pub use` 保持向后兼容。
